...
- ms-enforce Tier 2 FSM check timeout raised 90s → 600s to cover full FSM suite duration (~325s observed).
...
- `b94e9fe` feat(ms-enforce): add check_mypy() to Tier 1 (step 1.2.g)
...
- `014e0a7` ms-enforce: Core Rules oracle + Q3 fix + F821 backend bug fixed
...
- `5afdeca` fix: change default config_root to /srv/mediastack/config
