...
def test_py_migration_failure_rollback(tmp_path: Path, tmp_mig_dir: Path) -> None:
    ...
    Step 4.3 added migration 004 (audit_log). The broken-test migration
    is numbered 999 to sit AFTER all real migrations rather than
    colliding with one — every real migration commits, only 999 fails.
    """
    broken = tmp_mig_dir / "999_broken.py"
    broken.write_text(
        "import sqlite3\n\n"
    ...
    assert "999" in error_msg or "broken" in error_msg, (
    ...
    # All real migrations should have been committed before the broken one
    ...
    assert 4 in versions, "Migration 004 should be committed"

    # 999 (the broken test migration) MUST NOT be recorded
    assert 999 not in versions, "Broken migration 999 should not be recorded"
