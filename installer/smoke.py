...
_SPA_RE_TITLE = re.compile(r"mediastack", re.IGNORECASE)
...
def _http_get(
    ...
    """
    req = urllib.request.Request(url, headers={"User-Agent": "mediastack-smoke/5"})
...
def _check_systemd_active(
    ...
    *,
    ...
    while monotonic() < retry_deadline:
        ...
        if status_str == "failed":
            return SmokeTestResult(
                ...
                operator_message=(
                    "The mediastack service failed to start. "
                    "Recent backend logs may show why."
                ),
                diagnostic_command="journalctl -u mediastack.service -n 50 --no-pager",
            )
    ...
    return SmokeTestResult(
        ...
        operator_message=(
            "The mediastack service did not reach `active` state within 10 seconds. "
            "The unit may be slow to start or stuck in a startup hook."
        ),
        diagnostic_command="systemctl status mediastack.service",
    )


# ── P2: port bound by mediastack process ─────────────────────────────────────
...
def _check_port_bound(
    ...
    *,
    ...
    """P2: port is bound on 127.0.0.1 by the mediastack process.
        ...
        if bound_pid is not None:
            ...
            if bound_pid != main_pid:
                return SmokeTestResult(
                    ...
                    operator_message=(
                        f"Port {port} is bound, but not by the mediastack process. "
    ...
    return SmokeTestResult(
        ...
        operator_message=(
            f"The mediastack service is active but did not bind to port {port} "
            "within 5 seconds. The process may be initializing or another service "
            "may have the port."
        ),
        diagnostic_command=f"ss -ltnp && systemctl status mediastack.service",
    )
...
def _check_healthz(
    ...
    *,
    ...
    fail_result = SmokeTestResult(
        ...
        operator_message=(
            "The mediastack backend is running but `/healthz` did not respond "
...
def _check_startupz_and_readyz(
    ...
    *,
    ...
    for _n, delay in enumerate([0.0] + list(_BACKOFF_DELAYS)):
        ...
        if db_ping != "ok":
            ...
            return SmokeTestResult(
                ...
                operator_message=(
                    f"The mediastack backend cannot reach its database at "
                    f"{data_dir}/state.db. The data directory may have wrong "
                    "ownership or permissions."
                ),
                diagnostic_command=(
                    f"ls -la {data_dir}/ && "
                    "journalctl -u mediastack.service -n 50 --no-pager"
                ),
            )
    ...
    return SmokeTestResult(
        ...
        operator_message=(
            "The mediastack backend's startup did not complete within 10 seconds. "
            "The lifespan handler may be slow or stuck (commonly: database "
            "initialization, scheduler startup)."
        ),
        diagnostic_command="journalctl -u mediastack.service -n 100 --no-pager",
    )
    ...
    try:
        ...
        return SmokeTestResult(
            ...
            operator_message=(
                ...
                "mediastack SPA signature. This may indicate a misconfigured "
