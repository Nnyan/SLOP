...
class TestRenderTemplate:
    ...
    def test_install_dir_value_appears_in_exec_start(self):
        result = _render_template(_TMPL, "/srv/mediastack", "/data")
        assert "/srv/mediastack/.venv/bin/uvicorn" in result

    def test_data_dir_value_appears_in_environment(self):
        result = _render_template(_TMPL, "/opt/ms", "/var/lib/mediastack")
        assert "MS_DATA_DIR=/var/lib/mediastack" in result
...
class TestInstallServiceUnitFile:
    ...
    def test_unit_written_to_systemd_path(self):
        ...
        install_service("/opt/ms", "/var/lib/ms", **kwargs)
        path, _ = writes[0]
        assert path == Path("/etc/systemd/system/mediastack.service")
...
class TestInstallServiceSystemctl:
    ...
    def test_enable_targets_mediastack_unit(self):
        calls = []
        kwargs = _passing_kwargs()
        kwargs["run_systemctl"] = lambda sub, unit: calls.append((sub, unit))
        install_service("/opt/ms", "/var/lib/ms", **kwargs)
        enable_calls = [(s, u) for s, u in calls if s == "enable"]
        assert enable_calls == [("enable", "mediastack")]

    def test_start_targets_mediastack_unit(self):
        calls = []
        kwargs = _passing_kwargs()
        kwargs["run_systemctl"] = lambda sub, unit: calls.append((sub, unit))
        install_service("/opt/ms", "/var/lib/ms", **kwargs)
        start_calls = [(s, u) for s, u in calls if s == "start"]
        assert start_calls == [("start", "mediastack")]
...
class TestServiceBoundaryProbe:
    ...
    def test_read_template_raises_service_install_error_on_missing_file(self):
        ...
        with patch("installer.service._TEMPLATE_FILE",
                   new=Path("/nonexistent/mediastack.service.j2")):
