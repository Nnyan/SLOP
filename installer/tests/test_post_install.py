...
_TEMPLATE = (
    ...
    "\n"
    "Uninstall:  sudo <install_dir>/bin/mediastack uninstall\n"
)
...
class TestRender:
    def test_successful_substitution(self):
        ...
        assert "Mediastack v5.0.0 install complete" in result
        assert "http://192.168.1.10:8080/" in result
        assert "/opt/mediastack" in result
        assert "/var/lib/mediastack" in result
        assert "2026-01-01T00:00:00Z" in result

    def test_install_dir_appears_twice(self):
        result = render(**_RENDER_KWARGS)
        assert result.count("/opt/mediastack") == 2
    ...
    def test_uninstall_uses_wrapper_not_main_py(self):
        result = render(**_RENDER_KWARGS)
        assert "/bin/mediastack" in result
...
class TestWriteWrapper:
    def test_creates_wrapper_at_bin_mediastack(self, tmp_path):
        write_wrapper(tmp_path, run_chown=lambda p: None)
        assert (tmp_path / "bin" / "mediastack").exists()

    def test_wrapper_has_shebang(self, tmp_path):
        write_wrapper(tmp_path, run_chown=lambda p: None)
        content = (tmp_path / "bin" / "mediastack").read_text(encoding="utf-8")
