...
def test_composite_all_pass(tmp_path: Path) -> None:
    ...
    spec = {
        ...
        "checks": [
            ...
            {"type": "stat_match", "path": "post_install.stat", "mode": "644", "owner": "slop:slop"},
...
def _make_ubuntu_24_04_archive(distro_dir: Path) -> None:
    ...
    _write(distro_dir / "smoke_invocation.evidence", "Install complete. See /opt/slop/POST_INSTALL.txt")
    # post_install.txt: no placeholders
    _write(distro_dir / "post_install.txt", "SLOP v5.0.0 install complete\n\nhttp://192.168.1.100:8080/\n")
...
def test_check_distro_ubuntu_22_04_pass(tmp_path: Path, real_manifest: dict) -> None:
    ...
    _write(distro_dir / "data_dir.ls", "drwxr-x--- 2 slop slop 4096 May 16 12:00 /var/lib/slop")
    _write(distro_dir / "systemd.unit", "ExecStart=/opt/slop/.venv/bin/uvicorn backend.api.main:app --host 0.0.0.0 --port 8080\nEnvironment=MS_DATA_DIR=/var/lib/slop")
