...
def test_stdout_banner_in_final_third(tmp_path: Path) -> None:
    content = "A" * 600 + "Install complete. See /opt/mediastack/POST_INSTALL.txt"
...
def test_stdout_banner_in_first_third_fails(tmp_path: Path) -> None:
    content = "Install complete. See /opt/mediastack/POST_INSTALL.txt" + "A" * 600
    _write(tmp_path / "install.stdout.log", content)
    spec = {
        "type": "stdout_banner_position",
        "path": "install.stdout.log",
...
def test_stat_match_pass(tmp_path: Path) -> None:
    ...
    _write(tmp_path / "post_install.stat", "644 mediastack:mediastack 1024")
    spec = {
        "type": "stat_match",
        "path": "post_install.stat",
        "mode": "644",
        "owner": "mediastack:mediastack",
    }
    result = _run_check(tmp_path, spec, strict=True)
    assert result["passed"] is True
...
def test_stat_match_fail_mode(tmp_path: Path) -> None:
    _write(tmp_path / "post_install.stat", "600 mediastack:mediastack 1024")
    spec = {
        "type": "stat_match",
        "path": "post_install.stat",
        "mode": "644",
...
def test_regex_no_match_pass(tmp_path: Path) -> None:
    _write(tmp_path / "post_install.txt", "Mediastack v5.0.0 install complete\n")
...
def test_substring_match_pass(tmp_path: Path) -> None:
    ...
    spec = {"type": "substring_match", "path": "distro.os-release", "expected": "Debian GNU/Linux 12"}
    result = _run_check(tmp_path, spec, strict=True)
    assert result["passed"] is True
...
def test_composite_all_pass(tmp_path: Path) -> None:
    _write(tmp_path / "post_install.txt", "Mediastack v5.0.0 install complete\n")
    _write(tmp_path / "post_install.stat", "644 mediastack:mediastack 100")
    spec = {
        "type": "composite",
        "checks": [
            {"type": "file_exists", "path": "post_install.txt"},
            {"type": "stat_match", "path": "post_install.stat", "mode": "644", "owner": "mediastack:mediastack"},
...
def _make_ubuntu_24_04_archive(distro_dir: Path) -> None:
    ...
    banner = "==================================================\nInstall complete. See /opt/mediastack/POST_INSTALL.txt\n"
    _write(distro_dir / "install.stdout.log", preamble + banner)
    # smoke_invocation.evidence
    _write(distro_dir / "smoke_invocation.evidence", "Install complete. See /opt/mediastack/POST_INSTALL.txt")
    # post_install.txt: no placeholders
    _write(distro_dir / "post_install.txt", "Mediastack v5.0.0 install complete\n\nhttp://192.168.1.100:8080/\n")
    # post_install.stat: 644 mediastack:mediastack
    _write(distro_dir / "post_install.stat", "644 mediastack:mediastack 512")
...
def test_check_distro_debian_12_pass(tmp_path: Path, real_manifest: dict) -> None:
    ...
    _write(distro_dir / "data_dir.ls", "drwxr-x--- 2 mediastack mediastack 4096 May 16 12:00 /var/lib/mediastack")
    _write(distro_dir / "apt.python.dpkg", "ii  python3-venv  3.11.9 all\nii  python3  3.11.9 amd64")
...
def test_check_distro_ubuntu_22_04_pass(tmp_path: Path, real_manifest: dict) -> None:
    ...
    _write(distro_dir / "apt.sources.deadsnakes", "deb http://ppa.launchpad.net/deadsnakes/ppa/ubuntu jammy main")
    _write(distro_dir / "python3.resolution", "Python 3.11.9\n/usr/bin/python3.11\npython3 - priority 1")
    _write(distro_dir / "apt.python311.dpkg", "ii  python3.11-venv  3.11.9 amd64\nii  python3.11  3.11.9 amd64")
    _write(distro_dir / "data_dir.ls", "drwxr-x--- 2 mediastack mediastack 4096 May 16 12:00 /var/lib/mediastack")
    _write(distro_dir / "systemd.unit", "ExecStart=/opt/mediastack/.venv/bin/uvicorn backend.api.main:app --host 0.0.0.0 --port 8080\nEnvironment=MS_DATA_DIR=/var/lib/mediastack")
...
def test_evidence_hash_in_result(tmp_path: Path, real_manifest: dict) -> None:
    ...
    hashes = [
        v.get("evidence_hash") for v in result["core_predicates"].values()
