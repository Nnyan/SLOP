...
_INSTALL_DIR = Path("/opt/mediastack")
_DATA_DIR = Path("/var/lib/mediastack")
...
class TestReadState:
    def test_none_return_emits_no_install_detected_message(self):
        ...
        assert any("No v5 mediastack install detected" in e for e in errors)
...
class TestRunUninstallPipeline:
    ...
    def test_u4_failure_is_late_exits_1(self):
        user_entry = "mediastack:x:50:50::/nonexistent:/usr/sbin/nologin"
        seq = [
            _proc(0),  # stop
            _proc(0),  # disable
            _proc(0),  # rm unit
    ...
    def test_u4b_failure_is_late_exits_1(self):
        seq = [
            ...
            _proc(0),  # rm install_dir
            _proc(1),  # getent passwd → user absent; userdel skipped
            _proc(0, "mediastack:x:999:mediastack"),  # getent group → present, no extra
    ...
    def test_a6_user_mismatch_skips_userdel_continues(self):
        ...
        user_entry = "mediastack:x:1001:1001::/home/mediastack:/bin/bash"
        seq = [
            _proc(0),                        # stop
            _proc(0),                        # disable
            _proc(0),                        # rm unit
    ...
    def test_a65_group_mismatch_skips_groupdel_continues(self):
        ...
        group_entry = "mediastack:x:999:thirdparty"
        seq = [
            _proc(0),                         # stop
            _proc(0),                         # disable
            _proc(0),                         # rm unit
    ...
    def test_group_present_no_extra_groupdel_exits_0(self):
        ...
        user_entry = "mediastack:x:50:50::/nonexistent:/usr/sbin/nologin"
        group_entry = "mediastack:x:999:mediastack"
        seq = [
            _proc(0), _proc(0), _proc(0), _proc(0), _proc(0),
            _proc(0, user_entry),          # getent passwd → present, correct attrs
...
class TestClassPredictions:
    ...
    def test_a1_userdel_blocked_by_running_process(self):
        ...
        seq = [
            _proc(0), _proc(0), _proc(0), _proc(0), _proc(0),
            _proc(0, user_entry),   # getent passwd → present, correct attrs
            _proc(1, "user mediastack is currently used by process 999"),  # userdel blocked
            _proc(1),               # getent group → absent; groupdel skipped
        ]
        rc, errors, _ = _run_uninstall(run_seq=seq)
        assert rc == 1
    ...
    def test_s1_toctou_guard_data_dir_mismatch(self):
        ...
        sf = _make_state(data_dir="/var/lib/mediastack")
        errors = []
        printed = []
    ...
    def test_s6_group_extra_members_skips_groupdel(self):
        ...
        group_entry = "mediastack:x:999:thirdparty,otheruser"
        seq = [
            _proc(0), _proc(0), _proc(0), _proc(0), _proc(0),
            _proc(1),                   # getent passwd → user absent; userdel skipped
...
class TestRunClean:
    ...
    def test_c8_url_shown_on_success(self):
        ...
        assert any("http://" in p or "Mediastack remains running" in p for p in printed)

    def test_c8_url_suppressed_on_failure(self):
        seq = [_proc(0, "active"), _proc(0, "myapp-cont\tmyapp")]
        api_results = [{"ok": False, "steps": [], "error": "removal failed"}]
        _, _, printed = _run_clean(run_seq=seq, api_results=api_results)
        url_lines = [p for p in printed if "http://" in p and "Mediastack remains running" in p]
        assert url_lines == []
...
class TestVerifyRemovedUninstall:
    ...
    def test_u2_unit_absent_holds(self):
        with tempfile.TemporaryDirectory() as td:
            fake_unit = str(Path(td) / "mediastack.service")  # does not exist
    ...
    def test_u2_unit_present_fails(self):
        with tempfile.TemporaryDirectory() as td:
            fake_unit = Path(td) / "mediastack.service"
    ...
    def test_u4_user_present_system_attrs_fails(self):
        user_entry = "mediastack:x:999:999::/nonexistent:/usr/sbin/nologin"
