...
class TestCheckPrereqs:
    def test_all_pass_returns_seven_findings(self):
    ...
    def test_all_fail_still_returns_seven_findings(self):
        """No short-circuit: all 7 checks run even when every one fails."""
        findings = check_prereqs(
    ...
    def test_findings_order(self):
        """kernel, disk, backend-port, 80, 443, root, systemd — in that order."""
        findings = check_prereqs(**_passing_kwargs())
