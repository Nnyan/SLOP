...
class TestInstallHandoffF1:
    ...
    def test_full_handoff_set(self):
        ...
        args = build_parser().parse_args([
            "install",
            "--install-dir=/opt/mediastack",
            "--data-dir=/var/lib/mediastack",
            "--install-docker=yes",
            "--force",
        ])
        assert args.install_dir == "/opt/mediastack"
        assert args.data_dir == "/var/lib/mediastack"
        assert args.install_docker == "yes"
        assert args.force is True
...
class TestStateMachineS5:
    ...
    def test_s5_no_force_refuses(self, capsys):
        ...
        assert result == 1
        out = capsys.readouterr().out
        assert "no mediastack v5 state file" in out
...
class TestUserAttrsPreflight:
    ...
    def _mismatch(self):
        raise InstallUserMismatchError("user `mediastack` has wrong shell '/bin/bash'")
