...
install_service() renders installer/templates/mediastack.service.j2, writes it
to /etc/systemd/system/mediastack.service, then runs systemctl daemon-reload,
enable, and start.
...
_UNIT_PATH: Path = Path("/etc/systemd/system/mediastack.service")
_UNIT_NAME: str = "mediastack"
_TEMPLATE_FILE: Path = Path(__file__).parent / "templates" / "mediastack.service.j2"
...
def install_service(
    ...
    *,
    ...
    """Render the systemd unit template and activate the mediastack service.

    Writes install_dir and data_dir into the template, installs the unit at
    /etc/systemd/system/mediastack.service, then runs:
      systemctl daemon-reload
      systemctl enable mediastack
      systemctl start mediastack
