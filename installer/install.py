...
def detect_existing_install(
    ...
    *,
    ...
    """Detect the state of any existing mediastack install on this host.
    ...
    if existing is not None and existing.phase == "installed":
        ...
        if post_install_present and not smoke_passed:
            ...
            _state_label = "s2b"
            _message = (
                f"mediastack {existing.mediastack_version} was installed at "
                f"{install_dir}\n"
                ...
                f"{existing.data_dir}),\n"
                "or check `journalctl -u mediastack.service` for the original "
                "failure.\n"
            ...
            _state_label = "s2a"
            _message = (
                f"mediastack {existing.mediastack_version} is already installed at\n"
                f"{install_dir} (installed {existing.completed_at}).\n"
                f"Re-run with --force to reinstall (this preserves "
                f"{existing.data_dir}).\n"
                "Or use `mediastack uninstall` to remove first."
            )
        else:
    ...
    if existing is not None and existing.phase == "installing":
        ...
        return ExistingInstallDetection(
            state="s3",
            message=(
                "A previous mediastack install was interrupted\n"
                f"(started {existing.started_at}, never completed).\n"
                "Re-run with --force to clean and retry.\n"
                "Check `journalctl -u mediastack` for diagnostics from the "
                "previous attempt."
            ),
            forceable=True,
            existing=existing,
    ...
    if existing is None and install_dir_exists(install_dir):
        ...
        return ExistingInstallDetection(
            state="s5",
            message=(
                f"{install_dir} exists but no mediastack v5 state file is present.\n"
