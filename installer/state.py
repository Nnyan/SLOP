...
class StateFile:
    ...
    schema_version: int
    mediastack_version: str
    phase: str          # "installing" | "installed"
    started_at: str     # ISO 8601 UTC
    completed_at: Optional[str]   # ISO 8601 UTC; null while phase="installing"
...
def read_state_file(path: str | Path) -> Optional[StateFile]:
    ...
    try:
        ...
        raise StateFileCorruptedError(
            f"Cannot read state file at {p}: {exc}"
        ) from exc
    ...
    try:
        ...
        raise StateFileCorruptedError(
            f"State file at {p} is not valid JSON: {exc}"
        ) from exc
    ...
    for field in (
        "mediastack_version", "phase", "started_at",
        "install_dir", "data_dir", "install_user", "distro", "distro_version",
    ):
        val = data.get(field)
        if not isinstance(val, str):
            raise StateFileCorruptedError(
                f"State file at {p}: '{field}' must be a string; "
    ...
    if phase not in _VALID_PHASES:
        raise StateFileCorruptedError(
            f"State file at {p}: 'phase' must be one of "
    ...
    if not isinstance(port, int) or isinstance(port, bool):
        raise StateFileCorruptedError(
            f"State file at {p}: 'port' must be an integer; "
            f"got {type(port).__name__!r}"
        )
    ...
    return StateFile(
        ...
        mediastack_version=data["mediastack_version"],
        phase=phase,
...
def write_state_file(state: StateFile, path: str | Path) -> None:
    ...
    # TODO(Tier 4): state file is written as root with no explicit chown/chmod.
    # ADR 0013 §1 specifies final-state 0640; three ownership models are
    # evaluated in docs/cleanup/TIER_4_HANDOFF.md §O2 before choosing one.
