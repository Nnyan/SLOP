...
def run_install_pipeline(
    ...
    *,
    ...
    stop_service: Callable[[], None] = lambda: _run_mod.run_required(["systemctl", "stop", "mediastack.service"]),
) -> int:
    ...
    pre_state = state.StateFile(
        schema_version=1,
        mediastack_version=_INSTALLER_VERSION,
        phase="installing",
        started_at=now,
        completed_at=None,
    ...
    post_state = state.StateFile(
        schema_version=1,
        mediastack_version=resolved_version or _INSTALLER_VERSION,
        phase="installed",
    ...
    rendered = _post_install_mod.render(
        version=smoke_state.mediastack_version,
        hostname=hostname,
        port=port,
        install_dir=str(install_dir_path),
        data_dir=str(data_dir_path),
...
def _cmd_status(args: argparse.Namespace) -> int:
    return subprocess.run(["systemctl", "status", "mediastack.service"]).returncode
...
def _add_path_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--install-dir",
        metavar="PATH",
        help="Directory to install mediastack code",
    )
    parser.add_argument(
        "--data-dir",
        metavar="PATH",
        help="Directory for mediastack runtime data",
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="mediastack-installer",
        description="Mediastack v5 installer",
    )
    sub = parser.add_subparsers(dest="command", metavar="COMMAND")
    sub.required = True

    # install
    p_install = sub.add_parser("install", help="Install mediastack on this host")
    ...
    p_uninstall = sub.add_parser(
        "uninstall",
        help="Remove mediastack code, service, and user (preserves data directory)",
    )
    ...
    p_clean = sub.add_parser(
        "clean",
        help="Remove managed containers and their data; leave mediastack itself running",
    )
