...
_SYSTEMD_UNIT = "/etc/systemd/system/mediastack.service"
_MS_USER = "mediastack"
...
class UninstallUserMismatchError(Exception):
    """§A.6 carve-out: mediastack user has unexpected attributes."""


class GroupHasUnexpectedMembersError(Exception):
    """§A.6.5 carve-out: mediastack group has members the installer did not add."""
...
def _resolve_install_dir(args) -> Path:
    ...
    raw = (
        ...
        or os.environ.get("MEDIASTACK_INSTALL_DIR")
...
def _read_state(
    ...
    *,
    ...
    if sf is None:
        err_fn(
            f"No v5 mediastack install detected at {install_dir}.\n\n"
...
def _run_removal_pipeline(
    ...
    *,
    ...
    print_fn("Stopping mediastack.service...")
    result = run(["systemctl", "stop", "mediastack.service"])
    if result.returncode != 0:
        ...
        if unit_check.returncode != 0:
            ...
            err_fn(
                "The mediastack service did not stop. The unit may have a `KillMode=`\n"
                "configuration that prevents clean shutdown, or a child process may be unkillable.\n"
                "Diagnostic: `systemctl status mediastack.service` and `ps -ef | grep mediastack`"
            )
            return 1  # U1 fail → pipeline stops

    # Step 4: disable service (idempotent; ignore exit code)
    run(["systemctl", "disable", "mediastack.service"])
    ...
    if _usr is not None:
        ...
        if uid >= _SYSTEM_UID_CEILING or shell != _EXPECTED_SHELL or home != _EXPECTED_HOME:
            ...
            if result.returncode != 0:
                err_fn(
                    "The `mediastack` user could not be removed. The user may have a running\n"
                    "process (login session, cron job, lingering systemd user services), or a\n"
                    "system policy may be blocking `userdel`.\n"
                    "Diagnostic: `loginctl user-status mediastack` and `ps -u mediastack`"
                )
    ...
    if _grp is not None:
        ...
        if extra:
            ...
            if result.returncode != 0:
                err_fn(
                    "The `mediastack` group could not be removed.\n"
                    "Diagnostic: `getent group mediastack` and `journalctl -t groupdel -n 50`"
                )
    ...
    container_ids = _enumerate_docker_objects(
        run, ["docker", "ps", "-a", "--filter", "label=mediastack.managed=true",
              "--format", "{{.ID}}"],
        err_fn, "U6",
    ...
    if container_ids is not None and container_ids:
        ...
        if result.returncode != 0:
            err_fn(
                ...
                "Diagnostic: `docker ps -a --filter label=mediastack.managed=true` "
    ...
    volume_names = _enumerate_docker_objects(
        run, ["docker", "volume", "ls", "--filter", "label=mediastack.managed=true",
              "--format", "{{.Name}}"],
        err_fn, "U7",
...
def _uninstall_prompt(sf: StateFile, install_dir: Path, data_dir: Path) -> str:
    return (
        f"About to uninstall mediastack v{sf.mediastack_version}:\n\n"
        f"  - Stop and disable mediastack.service\n"
        f"  - Remove {_SYSTEMD_UNIT}\n"
        f"  - Remove {install_dir} (code, venv, frontend, state file, POST_INSTALL.txt)\n"
        f"  - Remove user 'mediastack' and group 'mediastack' (if installer-installed)\n\n"
...
def _purge_prompt(sf: StateFile, install_dir: Path, data_dir: Path, hostname: str) -> str:
    return (
        f"About to PURGE mediastack v{sf.mediastack_version} from {hostname}:\n\n"
        f"  - Stop and disable mediastack.service\n"
        f"  - Remove {_SYSTEMD_UNIT}\n"
        f"  - Remove {install_dir} (code, venv, frontend, state file, POST_INSTALL.txt)\n"
        f"  - Remove user 'mediastack' and group 'mediastack' (if installer-installed)\n"
        f"  - Remove {data_dir} (state.db, .env, per-app configs, compose fragments)\n"
        f"  - Remove ALL mediastack-managed Docker containers (label: mediastack.managed=true)\n"
        f"  - Remove ALL mediastack-managed Docker volumes (label: mediastack.managed=true)\n\n"
...
def verify_removed(
    ...
    *,
    ...
    skipped: list[str] = []
    diagnostics: dict[str, str] = {
        "U1": "systemctl status mediastack.service",
        "U2": f"ls -la {_SYSTEMD_UNIT}",
        "U3": f"lsof +D {install_dir}",
        "U4": "loginctl user-status mediastack && ps -u mediastack",
        "U4b": "getent group mediastack && journalctl -t groupdel -n 50",
        "U5a": f"lsof +D {data_dir} (check for mutation, not removal)",
        "U5b": f"lsof +D {data_dir} && mount | grep {data_dir}",
        "U6": "docker ps -a --filter label=mediastack.managed=true",
        "U7": "docker volume ls --filter label=mediastack.managed=true",
    }

    # U1: service inactive or unknown
    result = run(["systemctl", "is-active", "mediastack.service"])
    ...
    if result_u4b.returncode != 0:
        ...
        if extra and mode in ("uninstall", "purge"):
            ...
            # Group gone or only mediastack user (or clean mode) — acceptable per §B.2
    ...
    if mode in ("purge", "clean"):
        result = run([
            "docker", "ps", "-a",
            "--filter", "label=mediastack.managed=true",
            "--format", "{{.Names}}",
        ])
...
def _clean_prompt(
    ...
    lines = [f"About to reset all managed mediastack apps on {hostname}:\n"]
    for k in app_keys:
        lines.append(f"  - {k}")
    lines.append(
        "\nFor each app:\n"
