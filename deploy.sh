#!/usr/bin/env bash
# deploy.sh — Deploy Mediastack v3 to an Ubuntu 24.04 server
#
# Usage:
...
#   6. Installs and enables the mediastack systemd service
...
if [[ ! -f "$ENV_FILE" ]]; then
  cat > "$ENV_FILE" << ENVEOF
# Mediastack v3 — Configuration and secrets
# Edit this file, then run: sudo systemctl restart mediastack
#
...
$SUDO systemctl enable mediastack
$SUDO systemctl restart mediastack
...
for i in $(seq 1 20); do
  ...
  sleep 1
  [[ $i -eq 20 ]] && warn "API did not respond after 20s — check: journalctl -u mediastack -f"
done
...
echo
echo -e "  ${BOLD}Mediastack v3 deployed!${RESET}"
echo -e "  UI:     ${CYAN}${API_URL}${RESET}"
echo -e "  Logs:   ${CYAN}journalctl -u mediastack -f${RESET}"
