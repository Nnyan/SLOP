...
# intent auditable).
_DEFAULT_INSTALL_DIR="/opt/mediastack"
_DEFAULT_DATA_DIR="/var/lib/mediastack"

# ── Step 1: Parse arguments into environment variables ───────────────────────
MS_INSTALL_DIR="${MEDIASTACK_INSTALL_DIR:-${_DEFAULT_INSTALL_DIR}}"
MS_DATA_DIR="${MEDIASTACK_DATA_DIR:-${_DEFAULT_DATA_DIR}}"
MS_INSTALL_DOCKER=""
MS_VERSION_REF=""
MS_FORCE=0
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --install-dir)      MS_INSTALL_DIR="$2"; shift ;;
    --data-dir=*)       MS_DATA_DIR="${1#*=}" ;;
    --data-dir)         MS_DATA_DIR="$2"; shift ;;
    --install-docker=*) MS_INSTALL_DOCKER="${1#*=}" ;;
    --install-docker)   MS_INSTALL_DOCKER="$2"; shift ;;
    --version-ref=*)    MS_VERSION_REF="${1#*=}" ;;
    --version-ref)      MS_VERSION_REF="$2"; shift ;;
    --force)            MS_FORCE=1 ;;
    *)                  ;;
  esac
  shift
done
export MS_INSTALL_DIR MS_DATA_DIR MS_INSTALL_DOCKER MS_VERSION_REF MS_FORCE

# ── Step 2: Detect whether stdin is a TTY ────────────────────────────────────
if [ -t 0 ]; then
...
# ── Step 3: Pipe-mode fail-fast — before any filesystem write or apt call ────
#
...
# ── Step 4: Detect distro (read-only) ────────────────────────────────────────
...
# ── Step 5: Bootstrap python3 (3.11+), python3-venv, and git ─────────────────
_needs_apt_update=0
_MS_PYTHON3=python3
