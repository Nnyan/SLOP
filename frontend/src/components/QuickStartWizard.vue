<template>
  <div v-if="wizard.show && !dismissed" class="mb-4">
      ...
      <div class="card-body !pb-2">
        ...
        <div class="flex items-start justify-between mb-3">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <div class="text-xs text-slate-400">{{ wizard.percent }}%</div>
            <div class="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div class="h-full bg-sky-500 rounded-full transition-all"
                :style="`width:${wizard.percent}%`"/>
            </div>
            <button @click="dismiss" class="text-slate-300 hover:text-slate-500 ml-1 text-lg leading-none">✕</button>
          </div>
        </div>

        <!-- Phase grid -->
        <div class="grid grid-cols-4 gap-1.5 mb-2">
          <div v-for="phase in wizard.phases" :key="phase.id"
            ...
            <div class="flex items-center gap-1.5">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="phase.optional" class="text-xs text-slate-400 leading-none">optional</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activePhase" class="border-t border-sky-100 pt-2 mb-1">
          <div v-for="phase in wizard.phases.filter((p: any) => p.id === activePhase)" :key="phase.id"
            class="flex items-center gap-3">
            <div class="flex-1">
              <div class="text-sm font-medium text-slate-800">{{ phase.label }}</div>
              <div class="text-xs text-slate-500">{{ phase.description }}</div>
            </div>
            <div class="flex gap-1.5 shrink-0">
              <button v-if="phase.status !== 'complete'"
                @click="markPhase(phase.id, 'complete')"
                class="btn-primary btn-sm text-xs">
                Mark done
              </button>
              <RouterLink v-if="phase.route"
                :to="phase.route"
                class="btn-secondary btn-sm text-xs">
                Go →
              </RouterLink>
              <button v-if="phase.optional && phase.status !== 'skipped'"
                @click="markPhase(phase.id, 'skipped')"
                class="btn-secondary btn-sm text-xs text-slate-400">
                Skip
              </button>
              <button v-if="phase.status !== 'pending'"
        ...
        <div class="flex items-center justify-between pt-1 border-t border-sky-100">
          <button @click="dismiss" class="text-xs text-slate-400 hover:text-slate-600">
