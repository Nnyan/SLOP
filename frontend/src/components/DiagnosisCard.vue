<template>
  <div v-if="!dismissed" class="rounded-lg border border-amber-200 bg-amber-50 p-4 shadow-sm">
    <!-- Header row -->
    ...
    <!-- Problem -->
    <p class="text-sm text-slate-700 mb-2">{{ problem }}</p>

    <!-- Suggested fix -->
    <div class="mb-3">
      <p class="text-xs font-semibold text-amber-800 mb-1">Suggested fix:</p>
      <p class="text-sm text-slate-700 whitespace-pre-wrap font-mono text-xs leading-relaxed bg-white rounded border border-amber-100 px-3 py-2">{{ suggestedFix }}</p>
    </div>

    <!-- Amber banner for 501 response -->
    <div v-if="applyBanner" class="text-xs bg-amber-100 border border-amber-200 rounded px-3 py-2 text-amber-800 mb-3">
      {{ applyBanner }}
    </div>
    ...
    <div class="flex items-center gap-2">
      ...
      >
        <span v-if="applying" class="flex items-center gap-1">
