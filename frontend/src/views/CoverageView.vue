<template>
  <div class="p-6 max-w-screen-xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold text-slate-900">Topology & Coverage</h1>
      ...
      <div class="flex items-center gap-3">
        ...
        <button @click="refresh" :disabled="loading"
          ...
          <span v-if="loading" class="w-3 h-3 border-2 border-slate-300 border-t-sky-500 rounded-full animate-spin"/>
          <span v-else>↻</span>
          Refresh
        </button>
      </div>
    ...
    <div v-if="loading && !data" class="space-y-4">
      <div class="grid grid-cols-4 gap-4">
        <div v-for="n in 4" :key="n" class="h-24 bg-slate-100 rounded-xl animate-pulse"/>
      </div>
      <div class="h-96 bg-slate-100 rounded-xl animate-pulse"/>
    </div>

    <!-- Error -->
    <div v-else-if="error" class="rounded-xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">
      {{ error }}
      <button @click="refresh" class="ml-3 underline">Retry</button>
    </div>
    ...
    <template v-else-if="data">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          <div class="text-2xl font-bold" :class="coveragePctColor">{{ data.summary.coverage_pct }}%</div>
          <div class="text-xs text-slate-500 mt-1">Overall coverage</div>
          <div class="text-xs text-slate-400">{{ data.summary.covered }}/{{ data.summary.total }} nodes</div>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          <div class="text-2xl font-bold" :class="data.summary.critical_gaps ? 'text-red-600' : 'text-green-600'">
            {{ data.summary.critical_gaps }}
          </div>
          <div class="text-xs text-slate-500 mt-1">Critical gaps</div>
          <div class="text-xs text-slate-400">{{ data.summary.high_gaps }} high-risk</div>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          <div class="text-2xl font-bold text-slate-700">{{ data.nodes.length }}</div>
          <div class="text-xs text-slate-500 mt-1">Total nodes</div>
          <div class="text-xs text-slate-400">{{ Object.keys(data.summary.by_kind).length }} kinds</div>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          <div class="text-2xl font-bold text-slate-700">{{ data.known_bugs?.filter(b => b.fixed).length ?? 0 }}</div>
          <div class="text-xs text-slate-500 mt-1">Bugs fixed via tests</div>
          <div class="text-xs text-slate-400">tracked regressions</div>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div class="flex items-center gap-2 px-4 py-3 border-b border-slate-100 flex-wrap">
          <button v-for="kind in allKinds" :key="kind"
          ...
          <div class="ml-auto flex items-center gap-2">
            <input v-model="search" type="text" placeholder="Search nodes…"
              class="input text-xs w-48" />
            <button @click="toggleCoverageFilter"
              :class="['text-xs px-2.5 py-1 rounded-full border transition-colors',
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-slate-50 text-xs text-slate-500">
                ...
                <th class="px-4 py-2 text-left font-medium">Risk</th>
                <th class="px-4 py-2 text-left font-medium">Coverage</th>
                <th class="px-4 py-2 text-left font-medium">Test types</th>
            ...
            <tbody>
              <tr v-for="node in filteredNodes" :key="node.id"
                ...
                <td class="px-4 py-2">
                  <div class="font-mono text-xs text-slate-700 truncate max-w-xs" :title="node.label">
                    {{ node.label }}
                  </div>
                  <div v-if="node.detail" class="text-xs text-slate-400 truncate max-w-xs">{{ node.detail }}</div>
                </td>
                <td class="px-4 py-2">
                ...
                <td class="px-4 py-2">
                  <div class="flex gap-1 flex-wrap">
                    <span v-for="t in node.test_kinds" :key="t"
                ...
                <td class="px-4 py-2">
                  <span class="text-xs text-slate-400 font-mono truncate max-w-xs block" :title="node.file">
      ...
      <div v-if="selectedNode"
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            <div class="font-mono text-sm font-medium text-slate-800">{{ selectedNode.label }}</div>
            <div class="text-xs text-slate-500 mt-0.5">{{ selectedNode.detail }}</div>
          </div>
          <button @click="selectedNode = null" class="text-slate-400 hover:text-slate-600 text-lg">✕</button>
        </div>
        <div class="grid grid-cols-3 gap-4 text-xs">
          <div>
            <div class="text-slate-400 mb-1">Kind</div>
            <span :class="['px-1.5 py-0.5 rounded', kindColors[selectedNode.kind]?.badge ?? 'bg-slate-100 text-slate-600']">
              {{ selectedNode.kind }}
            </span>
          </div>
          <div>
            <div class="text-slate-400 mb-1">Risk</div>
            <span :class="['font-medium', riskColor(selectedNode.risk)]">{{ selectedNode.risk }}</span>
          </div>
          <div>
            <div class="text-slate-400 mb-1">Coverage</div>
        ...
        <div v-if="selectedNode.test_files.length" class="text-xs">
          <div class="text-slate-400 mb-1">Test files</div>
          <div class="flex gap-1 flex-wrap">
            <span v-for="f in selectedNode.test_files" :key="f"
              class="bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-mono">{{ f }}</span>
          </div>
        </div>
        <div v-if="selectedNode.depends_on.length" class="text-xs">
          <div class="text-slate-400 mb-1">Depends on</div>
          <div class="text-slate-600">{{ selectedNode.depends_on.slice(0, 5).join(', ') }}</div>
        </div>
        <div class="text-xs">
          <div class="text-slate-400 mb-1">Source</div>
          <span class="font-mono text-slate-600">{{ selectedNode.file }}:{{ selectedNode.line }}</span>
        </div>
        <div v-if="!selectedNode.covered" class="bg-amber-50 border border-amber-200 rounded p-3 text-xs text-amber-700">
          <strong>Gap:</strong> {{ gapSuggestion(selectedNode) }}
        </div>
      </div>

      <!-- Known bugs timeline -->
      <div v-if="data.known_bugs?.length" class="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <h2 class="text-sm font-medium text-slate-700">Bugs Fixed via Testing</h2>
          <span class="text-xs text-slate-400">{{ data.known_bugs.filter(b => b.fixed).length }} fixed</span>
        </div>
        <div class="divide-y divide-slate-50">
          <div v-for="bug in data.known_bugs" :key="bug.id"
            class="px-4 py-3 flex items-start gap-3">
            <span :class="['text-base shrink-0 mt-0.5', bug.fixed ? 'text-green-500' : 'text-amber-400']">
              {{ bug.fixed ? '✓' : '○' }}
            </span>
            <div class="flex-1 min-w-0">
              <div class="text-xs text-slate-700">{{ bug.desc }}</div>
              <div class="text-xs text-slate-400 mt-0.5 font-mono">{{ bug.id }}</div>
            </div>
            <div class="text-xs font-mono text-slate-400 shrink-0">{{ bug.commit?.slice(0,7) ?? '' }}</div>
          </div>
        </div>
      </div>
    </template>
