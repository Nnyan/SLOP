  ...
  <div class="p-4 max-w-4xl mx-auto w-full">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="page-subtitle">Configure debrid vs. download paths per media type</p>
      </div>
    </div>

    <!-- How it works — slide-in panel to right of main column -->
    <Teleport to="body">
      <div v-if="showHowItWorks" class="fixed inset-0 z-40" @click.self="showHowItWorks = false">
        <div class="absolute right-0 top-0 h-full w-80 bg-white border-l border-slate-200 shadow-xl flex flex-col">
          <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <button @click="showHowItWorks = false" class="text-slate-400 hover:text-slate-600">✕</button>
          </div>
    ...
    <div class="mb-6">
      <h2 class="font-semibold text-slate-900 mb-3">Installed Arr Instances</h2>
      <div v-if="!allInstances.length" class="card card-body text-sm text-slate-400 text-center py-6">
        No multi-instance routing configured yet. Deploy a debrid instance below to get started.
      </div>
      <div v-else class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="inst in allInstances" :key="inst.instance_key"
          ...
          <div class="flex-1 min-w-0">
            <div class="text-sm font-medium text-slate-900 truncate">{{ inst.label }}</div>
            <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
              <span class="badge text-xs" :class="roleBadge(inst.role)">{{ inst.role }}</span>
              <span class="text-xs text-slate-400">{{ inst.manifest_key }}</span>
              <span v-if="inst.host_port" class="text-xs text-slate-400">:{{ inst.host_port }}</span>
            </div>
          </div>
          <button @click="confirmRemoveInstance(inst)" class="text-slate-300 hover:text-red-400 shrink-0">✕</button>
        </div>
      </div>
    </div>

    <!-- Per-media-type routing cards -->
    <h2 class="font-semibold text-slate-900 mb-3">Media Type Configuration</h2>
    <div class="space-y-3">
      <div v-for="route in routes" :key="route.media_type"
        class="card overflow-hidden">

        <!-- Card header row -->
        <div class="card-body !py-2.5 flex items-center justify-between cursor-pointer select-none"
          @click="toggleExpand(route.media_type)">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 flex items-center justify-center overflow-hidden shrink-0">
              <img :src="mediaTypeIconUrl(route.media_type)" class="w-7 h-7 object-contain"
            ...
            <div>
              <div class="font-medium text-slate-900 capitalize">{{ route.media_type }}</div>
              <div class="text-xs text-slate-400">{{ route.canonical_manifest }}</div>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <!-- Seerr badge -->
            <span v-if="route.seerr_supported" class="badge badge-blue text-xs">Seerr ✓</span>
            <span v-else class="badge badge-gray text-xs">Seerr n/a</span>

            <!-- Routing status -->
            <div class="text-xs text-right hidden sm:block">
              <div v-if="route.debrid_instance" class="text-sky-600">⚡ {{ route.debrid_instance }}</div>
              <div v-if="route.download_instance" class="text-slate-500">⬇ {{ route.download_instance }}</div>
              <div v-if="!route.debrid_instance && !route.download_instance" class="text-slate-300">not configured</div>
            </div>
        ...
        <!-- Expanded panel -->
        <div v-if="expanded === route.media_type" class="border-t border-slate-100 px-4 py-3 space-y-3">
          ...
          <div class="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="pendingRoutes[route.media_type].debrid_instance" class="input flex-1">
                  <option value="">None</option>
                  <option v-for="inst in debridInstances(route.canonical_manifest)"
                    :key="inst.instance_key" :value="inst.instance_key">
                    {{ inst.label }}
                  </option>
                </select>
              </div>
            ...
            <div>
              ...
              <select v-model="pendingRoutes[route.media_type].download_instance" class="input">
                <option value="">None</option>
                <option v-for="inst in downloadInstances(route.canonical_manifest)"
                  :key="inst.instance_key" :value="inst.instance_key">
                  {{ inst.label }}
                </option>
              </select>
          ...
          <div>
            ...
            <div class="flex gap-2">
              <label v-for="opt in ['debrid','download','ask']" :key="opt"
                ...
                <input type="radio" :value="opt"
                  v-model="pendingRoutes[route.media_type].default_path" class="sr-only"/>
          ...
          <!-- Save routing -->
          <div class="flex items-center gap-3">
            <button @click="saveRouting(route.media_type)"
              :disabled="savingRoute === route.media_type"
              class="btn-primary btn-sm">
              {{ savingRoute === route.media_type ? 'Saving…' : 'Save routing' }}
            </button>
            <span v-if="savedRoute === route.media_type" class="text-sm text-green-600">✓ Saved</span>
          </div>

          <!-- Seerr help -->
          <div v-if="route.seerr_supported" class="border-t border-slate-100 pt-4">
            <div class="flex items-center justify-between mb-2">
              <span class="section-title">Seerr configuration</span>
              <button @click="loadSeerrHelp(route.media_type)" class="text-xs text-sky-500 hover:text-sky-600">
                Load steps →
              </button>
            </div>
            <div v-if="seerrHelp[route.media_type]" class="space-y-2 text-sm text-slate-700">
              <div v-if="seerrHelp[route.media_type].seerr_steps">
                <div v-for="(step, i) in seerrHelp[route.media_type].seerr_steps" :key="i"
                  class="flex items-start gap-2">
                  <span class="font-mono text-xs text-slate-400 mt-0.5 shrink-0">{{ String(i+1).padStart(2,'0') }}</span>
                  <span>{{ step }}</span>
                </div>
              </div>
              <div v-if="seerrHelp[route.media_type].arr_steps" class="mt-3 pt-3 border-t border-slate-100">
                <div class="text-xs font-medium text-slate-500 mb-2">Arr download client setup</div>
                <div v-for="(step, i) in seerrHelp[route.media_type].arr_steps" :key="i"
          ...
          <div class="border-t border-slate-100 pt-4">
            ...
            <div class="grid sm:grid-cols-3 gap-3">
              <div>
                ...
                <select v-model="newInst[route.media_type].role" class="input">
              ...
              <div>
                ...
                <input v-model="newInst[route.media_type].label" class="input"
              ...
              <div>
                ...
                <input v-model.number="newInst[route.media_type].host_port" type="number"
                  class="input" placeholder="auto"/>
              </div>
            </div>
            <div v-if="deployErrors[route.media_type]"
              class="mt-2 text-sm text-red-600 bg-red-50 rounded p-2">
              {{ deployErrors[route.media_type] }}
            </div>
            <button @click="deployInstance(route)"
    ...
    <Teleport to="body">
      <div v-if="removeTarget" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/30 backdrop-blur-sm" @click="removeTarget = null"/>
        <div class="relative card w-full max-w-sm mx-4 card-body">
          <h3 class="font-semibold text-slate-900">Remove {{ removeTarget.label }}?</h3>
          ...
          <div class="flex gap-3 mt-4">
            ...
            <button @click="doRemoveInstance" class="btn-danger flex-1">Remove</button>
          </div>
        </div>
      </div>
    </Teleport>
...
import { routingCache, setRoutingCache } from '../appCache'
