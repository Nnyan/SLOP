<template>
  <div class="p-4 max-w-4xl mx-auto w-full">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="page-subtitle flex items-center gap-2">
          ...
          <span v-if="platformStore.isReady"
      ...
      <button @click="refreshAll" :disabled="loading" class="btn-secondary btn-sm">
        <svg v-if="!loading" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4">
          <polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/>
        </svg>
        <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 animate-spin">
    ...
    <!-- Setup prompt -->
    <div v-if="!platformStore.isReady" class="card mb-6">
      <div class="card-body flex items-start gap-4">
        <div class="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center shrink-0">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-5 h-5 text-amber-600">
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
          </svg>
        </div>
        <div>
          <h3 class="font-semibold text-slate-900">Platform setup required</h3>
          <p class="text-sm text-slate-500 mt-1">
            Complete the setup wizard to deploy Traefik, configure your domain, and start installing apps.
          </p>
          <RouterLink to="/setup" class="btn-primary btn-sm mt-3 inline-flex">
            Start setup →
          </RouterLink>
        </div>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div v-for="stat in stats" :key="stat.label" class="card card-body">
    ...
    <div class="mb-6">
      <div class="flex items-center justify-between mb-3">
        <h2 class="font-semibold text-slate-900">Installed Apps</h2>

      </div>

      <div v-if="installedApps.length === 0 && !loading" class="card card-body text-center py-12">
        <div class="text-slate-400 text-sm">No apps installed yet.</div>
      </div>

      <div v-if="loading && installedApps.length === 0" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="n in 3" :key="n" class="card card-body animate-pulse">
          <div class="flex items-start gap-3">
            <div class="w-9 h-9 bg-slate-100 rounded-lg shrink-0"/>
            <div class="flex-1 space-y-1.5"><div class="h-3 bg-slate-100 rounded w-1/2"/><div class="h-2.5 bg-slate-100 rounded w-1/3"/></div>
          </div>
        </div>
      </div>
      <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <RouterLink v-for="app in installedApps" :key="app.key"
          :to="`/apps/${app.key}`"
          ...
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 bg-slate-100 rounded-lg flex items-center justify-center overflow-hidden">
                <img :src="iconUrl(app.key)" :alt="app.display_name"
              ...
              <div>
                <div class="font-medium text-slate-900 text-sm">{{ app.display_name }}</div>
                <div class="text-xs text-slate-400 capitalize">{{ app.category }}</div>
              </div>
            </div>
<div class="flex flex-col items-end gap-1">
              <span :class="statusBadge(app.status)">{{ app.status }}</span>
              <span v-if="app.category === 'custom'" class="badge badge-yellow text-xs">custom</span>
            </div>
          </div>

          <!-- Health checks for this app -->
          <div v-if="appHealth[app.key]?.length" class="mt-3 space-y-1">
            <div v-for="check in appHealth[app.key]" :key="check.check_name"
    ...
    <div class="card card-body">
      <div class="flex items-center gap-3">
        <div class="w-8 h-8 bg-violet-100 rounded-lg flex items-center justify-center text-base shrink-0">🤖</div>
        <template v-if="llmStatus">
          <div class="flex-1">
            ...
            <div class="text-xs text-slate-400 mt-0.5">{{ llmStatus.description }}</div>
          </div>
          <RouterLink to="/models" class="btn-secondary btn-sm ml-auto shrink-0">Configure</RouterLink>
        </template>
        <template v-else>
