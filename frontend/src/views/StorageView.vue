  ...
  <div class="p-4 max-w-4xl mx-auto w-full">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="page-subtitle">External NAS, cloud storage, and remote filesystem connections</p>
      </div>
    </div>

    <!-- Host disk summary from system fingerprint -->
    <div v-if="hostDisks.length" class="card mb-4">
      <div class="card-body !py-2.5">
        ...
        <div class="space-y-1.5">
          <div v-for="disk in hostDisks" :key="disk.path" class="flex items-center gap-3">
    ...
    <div v-if="loadingInitial" class="space-y-2 animate-pulse">
      <div v-for="n in 2" :key="n" class="card card-body h-14 bg-slate-50"/>
    </div>
    <div v-else-if="sources.length === 0 && !showAdd" class="card card-body text-center py-12">
      <div class="text-4xl mb-3">💾</div>
      <div class="font-semibold text-slate-700">No storage sources configured</div>
      ...
      <button @click="showAdd = true" class="btn-primary btn-sm mt-4 inline-flex">Add storage source</button>
    </div>

    <div v-if="sources.length > 0 && !showAdd" class="flex justify-end mb-3">
      <button @click="showAdd = true" class="btn-secondary btn-sm">+ Add source</button>
    </div>
    <div v-else-if="sources.length > 0" class="mb-1"></div>
    <div v-if="sources.length > 0" class="space-y-3">
      <div v-for="src in sources" :key="src.id ?? src.name" class="card card-body">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="font-medium text-slate-900 flex items-center gap-2">
                {{ src.name }}
                <span v-if="src.is_primary" class="badge badge-blue text-xs">primary</span>
              </div>
              <div class="text-xs text-slate-400">{{ src.source_type.toUpperCase() }} · {{ src.mount_point }}</div>
              <div v-if="src.remote_host" class="text-xs text-slate-400">{{ src.remote_host }}{{ src.remote_path }}</div>
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button @click="verify(src.id!)" class="btn-secondary btn-sm">Verify</button>
            <button @click="getConfig(src.id!)" class="btn-ghost btn-sm">Config</button>
          </div>
        </div>
        <div v-if="src.error_message" class="mt-2 text-xs text-red-600 bg-red-50 rounded px-2 py-1">
          {{ src.error_message }}
        </div>
      </div>
    </div>
    ...
    <div v-if="showAdd" class="card mb-4">
      <div class="card-header flex items-center justify-between">
        ...
        <button @click="showAdd = false; addError = null" class="text-slate-400 hover:text-slate-600 text-sm">✕</button>
      </div>
      <div class="card-body space-y-3">
        <div class="grid sm:grid-cols-2 gap-3">
          <div>
            <label class="label">Name</label>
            <input v-model="form.name" class="input" placeholder="Main NAS" />
          </div>
          <div>
            <label class="label">Type</label>
            <select v-model="form.source_type" class="input">
              <option value="nfs">NFS (LAN NAS)</option>
              <option value="smb">SMB/CIFS (Synology, Windows)</option>
              <option value="rclone">rclone (S3, Backblaze, SFTP)</option>
              <option value="local">Local path (already mounted)</option>
            </select>
          </div>
          <div v-if="form.source_type !== 'local'">
            <label class="label">Remote host</label>
            <input v-model="form.remote_host" class="input" placeholder="10.0.1.100" />
          </div>
          <div v-if="['nfs','smb'].includes(form.source_type)">
            <label class="label">{{ form.source_type === 'nfs' ? 'Export path' : 'Share name' }}</label>
            <input v-model="form.remote_path" class="input" :placeholder="form.source_type === 'nfs' ? '/volume1/media' : 'media'" />
          </div>
          <div>
            <label class="label">Mount point</label>
            <input v-model="form.mount_point" class="input" placeholder="/mnt/nas01" />
          </div>
          <label class="flex items-center gap-2 cursor-pointer self-end pb-1">
            <input v-model="form.is_primary" type="checkbox" class="rounded border-slate-300" />
            <span class="text-sm text-slate-700">Primary media root</span>
          </label>
        </div>
        <div v-if="addError" class="text-sm text-red-600 bg-red-50 rounded p-2">{{ addError }}</div>
        <div class="flex gap-3 pt-1">
          <button @click="showAdd = false; addError = null" class="btn-secondary btn-sm">Cancel</button>
          <button @click="addSource" :disabled="adding" class="btn-primary btn-sm">{{ adding ? 'Adding…' : 'Add source' }}</button>
        </div>
      </div>
    </div>
    ...
    <!-- Config modal -->
    <Teleport to="body">
      <div v-if="configData" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/30 backdrop-blur-sm" @click="configData = null"/>
        <div class="relative card w-full max-w-xl mx-4">
          <div class="card-header flex items-center justify-between">
            <span class="font-semibold">Mount Configuration</span>
            <button @click="configData = null" class="text-slate-400 hover:text-slate-600">✕</button>
          </div>
          <div class="card-body space-y-4">
            <p class="text-sm text-slate-600">{{ configData.note }}</p>
            <div v-if="configData.systemd_unit">
              <div class="section-title mb-2">Systemd unit ({{ configData.systemd_unit_name }})</div>
            ...
            <div v-if="configData.install_steps?.length">
              <div class="section-title mb-2">Install steps</div>
              <div class="space-y-1">
                <div v-for="(step, i) in configData.install_steps" :key="i" class="flex items-start gap-2 text-sm">
