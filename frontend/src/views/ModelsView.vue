<template>
  <div class="p-4 max-w-4xl mx-auto w-full">
    <div class="flex items-baseline gap-3 mb-2">
      <h1 class="page-title">LLM Models</h1>
      <p class="text-sm text-slate-400">AI health agent — diagnose app failures</p>
    </div>
    ...
    <div class="card mb-2">
      <div class="card-body !py-2.5">
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <template v-if="ping">
                <span v-if="ping.reachable && ping.model_loaded" class="badge badge-green text-xs">ready</span>
                <span v-else-if="ping.reachable && !ping.model_loaded" class="badge badge-yellow text-xs">no model</span>
                <span v-else class="badge badge-red text-xs">offline</span>
              </template>

              <div class="flex items-center gap-2 ml-auto">
                <button @click="doPing" :disabled="pinging"
                  class="text-xs text-slate-400 hover:text-slate-600">
                  {{ pinging ? '…' : '↻' }}
                </button>
                <button @click="agentExpanded = !agentExpanded"
                  class="text-xs text-slate-400 hover:text-slate-600">
                  {{ agentExpanded ? '▲' : '▼' }}
                </button>
              </div>
          ...
          <!-- Expanded detail -->
          <div v-if="agentExpanded" class="mt-2 pt-2 border-t border-slate-100">
            <p v-if="!ping" class="text-sm text-slate-400 italic">Checking Ollama…</p>
            ...
            <template v-else-if="ping.reachable && ping.model_loaded">
              ...
              <div class="flex flex-wrap gap-1.5">
                <span v-for="m in ping.loaded_models" :key="m"
            ...
            <template v-else-if="ping.reachable && !ping.model_loaded">
              <p class="text-sm text-amber-700 font-medium">Ollama is running but the model isn't loaded.</p>
              <div class="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 space-y-1.5">
                <p class="text-xs text-amber-800">{{ ping.error }}</p>
                ...
                <p v-if="ping.loaded_models?.length" class="text-xs text-slate-400">
            ...
            <div v-if="cloudLLMConfigured" class="mt-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2">
            ...
            <div v-else-if="agentStatus?.last_error_type === 'dns' || agentStatus?.last_error_type === 'connection'"
              class="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
              <p class="text-xs text-amber-800 font-medium mb-1">Ollama is not installed</p>
              <p class="text-xs text-amber-700">{{ agentStatus?.description }}</p>
              <div class="mt-2 flex gap-2">
                <RouterLink to="/catalog" class="text-xs text-sky-600 font-medium hover:underline">
                  Go to Catalog →
                </RouterLink>
              </div>
            </div>
            <!-- Auth failure -->
            <div v-else-if="agentStatus?.last_error_type === 'auth'"
              class="mt-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2">
              <p class="text-xs text-red-800 font-medium mb-1">API key issue</p>
              <p class="text-xs text-red-700">{{ agentStatus?.description }}</p>
              <RouterLink to="/settings?tab=ai" class="text-xs text-sky-600 font-medium hover:underline">
            ...
            <template v-else>
              ...
              <div class="mt-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 space-y-2">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <input v-model="ollamaUrlInput" type="text"
                    class="input !py-0.5 text-xs font-mono flex-1"
                    placeholder="http://localhost:11434" />
                  <button @click="saveOllamaUrl" :disabled="savingUrl"
                    class="btn-secondary btn-sm text-xs shrink-0">
                    {{ savingUrl ? '…' : 'Save' }}
                  </button>
                </div>
                <p class="text-xs text-red-600">{{ ping.error }}</p>
            ...
            <!-- Agent cycle stats -->
            <div v-if="agentStatus && agentStatus.status !== 'unknown'"
        ...
        <div class="flex gap-2 mt-2 pt-2 border-t border-slate-100">
          <button @click="runAgent" :disabled="runningAgent" class="btn-primary btn-sm text-xs">
            {{ runningAgent ? 'Running…' : '▶ Run agent' }}
          </button>
          <button @click="evaluateAgent" :disabled="evaluating" class="btn-secondary btn-sm text-xs">
      ...
      <div v-if="evalSteps.length" class="mt-2 pt-2 border-t border-slate-100 space-y-1">
        <template v-for="step in evalSteps" :key="step.label">
          <!-- Model compatibility table — special render -->
          <div v-if="step.label === 'Model compatibility'" class="mt-2">
            <div class="text-xs font-medium text-slate-500 mb-1.5">Model compatibility</div>
            <div class="grid grid-cols-3 gap-1.5">
              <div v-for="row in parseModelCompat(step.detail)" :key="row.name"
        ...
        <div v-if="evalSummary" class="mt-1.5 pt-1.5 border-t border-slate-100 text-xs font-medium"
    ...
    <div v-if="needsHFToken" class="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 flex items-center gap-2">
      ...
      <span class="text-xs text-amber-800 flex-1">
        ...
        <RouterLink to="/settings" class="underline font-medium">add it in Settings → AI</RouterLink>.
      </span>
      <button @click="needsHFToken = false; downloadError = null"
        class="text-amber-400 hover:text-amber-600 shrink-0">✕</button>
    </div>

    <!-- Download error (non-401) -->
    <div v-else-if="downloadError" class="mb-4 rounded-xl border border-red-200 bg-red-50 p-3">
      <div class="flex items-center justify-between">
        <p class="text-sm text-red-800">{{ downloadError }}</p>
        <button @click="downloadError = null" class="text-xs text-red-400 hover:text-red-600 ml-3 shrink-0">✕</button>
      </div>
    </div>

    <!-- Installed models -->
    <div class="flex items-center justify-between mb-2">
      <h2 class="font-semibold text-slate-900">Installed Models</h2>
      <button @click="refreshModels" class="text-xs text-sky-500 hover:text-sky-600">Refresh</button>
    </div>


    <!-- Installed models skeleton while loading -->
    <div v-if="!modelsLoaded" class="grid grid-cols-2 gap-2 mb-3">
      <div v-for="n in 2" :key="n" class="card card-body !py-2.5 !px-3 animate-pulse">
        <div class="flex items-center gap-2">
    ...
    <div v-else class="grid grid-cols-2 gap-2 mb-3">
      <div v-for="m in installedModels" :key="m.filename" class="card card-body !py-2.5 !px-3">
        <div class="flex items-center gap-2">
          ...
          <div class="flex-1 min-w-0">
            <div class="font-mono text-xs text-slate-900 truncate font-medium">{{ m.filename }}</div>
            <div class="text-xs text-slate-400 flex items-center gap-1.5 mt-0.5">
              <span>{{ m.size_mb.toFixed(0) }}MB</span>
              <span v-if="m.path?.startsWith('ollama://')"
                class="px-1 py-0 rounded bg-sky-100 text-sky-600 text-xs">Ollama</span>
              <span v-else class="px-1 py-0 rounded bg-slate-100 text-slate-500 text-xs">local</span>
              <span v-if="!m.valid" class="text-red-500">✗</span>
            </div>
          </div>
          <div class="flex gap-1 shrink-0">
            <button @click="enableModel(m)"
            ...
            <button @click="removeModel(m.filename)" class="btn-secondary btn-sm text-xs text-red-500">
              Remove
            </button>
          </div>
        </div>
    ...
    <div class="card mb-3">
      <div class="card-header cursor-pointer flex items-center justify-between"
        ...
        <h2 class="font-semibold text-slate-900">Add Custom Model</h2>
        <span class="text-slate-400 text-sm">{{ showAddCustom ? '▲' : '▼' }}</span>
      </div>
      <div v-if="showAddCustom" class="card-body space-y-4">
        <!-- Tabs -->
        <div class="flex gap-2 border-b border-slate-100 pb-3">
          <button v-for="tab in ['URL', 'HuggingFace Search', 'Local file']" :key="tab"
            @click="addTab = tab"
        ...
        <div v-if="addTab === 'URL'" class="space-y-3">
          ...
          <!-- Preflight result -->
          <div v-if="preflight" :class="['rounded-lg p-3 text-sm', preflight.ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700']">
          ...
          <div class="flex gap-2">
            <button @click="runPreflight" :disabled="!customUrl || preflighting" class="btn-secondary btn-sm">
              {{ preflighting ? '…' : 'Validate URL' }}
            </button>
            <button @click="startCustomDownload" :disabled="!preflight?.ok || !!downloadingModel" class="btn-primary btn-sm">
              Download
            </button>
          </div>
        </div>
        ...
        <div v-if="addTab === 'HuggingFace Search'" class="space-y-3">
          <div class="flex gap-2">
            <input v-model="hfQuery" type="text" class="input flex-1" placeholder="phi-4 mini, llama, mistral…"
              @keyup.enter="searchHF" />
            <button @click="searchHF" :disabled="searching" class="btn-secondary btn-sm">
              {{ searching ? '…' : 'Search' }}
            </button>
          </div>
          <div v-if="hfResults.length" class="space-y-2">
            <div v-for="r in hfResults" :key="r.id"
              class="flex items-center gap-3 px-3 py-2 rounded-lg border border-slate-100 hover:border-slate-200">
              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-slate-900 truncate">{{ r.id }}</div>
                <div class="text-xs text-slate-400">{{ r.downloads?.toLocaleString() }} downloads</div>
              </div>
              <a :href="`https://huggingface.co/${r.id}`" target="_blank" rel="noopener"
                class="text-xs text-sky-500 hover:text-sky-600 shrink-0">View ↗</a>
            </div>
          </div>
        </div>

        <!-- Local file tab -->
        <div v-if="addTab === 'Local file'" class="space-y-3">
          <div>
            <label class="label">File path on server</label>
            <input v-model="localPath" type="text" class="input font-mono"
              placeholder="/mnt/media/models/my-model.gguf" />
            <p class="text-xs text-slate-400 mt-1">Absolute path to an existing .gguf file on the server</p>
          </div>
          <button @click="validateLocal" :disabled="!localPath || validatingLocal" class="btn-secondary btn-sm">
            {{ validatingLocal ? '…' : 'Validate & Register' }}
          </button>
          <div v-if="localValidation" :class="['text-sm rounded-lg p-3', localValidation.valid ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700']">
    ...
    <div class="card mb-3">
      <div class="card-body !py-2.5">
        <div class="flex items-center justify-between mb-2">
          ...
          <button @click="saveOllamaUrl" :disabled="savingUrl" class="btn-primary btn-sm text-xs">
        ...
        <div class="space-y-1 mb-2">
          <div v-for="grp in ['local','cloud']" :key="grp">
            <div class="text-xs text-slate-400 uppercase tracking-wider mb-0.5">{{ grp }}</div>
            <div class="flex gap-1 flex-wrap">
              <button v-for="p in PROVIDERS.filter(x=>x.group===grp)" :key="p.id"
        ...
        <div v-if="isCloudProvider && inferenceProvider !== 'openrouter'" class="flex items-center gap-2 mb-1">
        ...
        <div class="space-y-2">
          ...
          <div v-if="inferenceProvider !== 'openrouter'" class="flex items-center gap-2">
            <label class="text-xs text-slate-500 w-10 shrink-0">URL</label>
            <input v-model="ollamaUrlInput" type="text" class="input text-xs font-mono flex-1"
              :placeholder="providerUrl(inferenceProvider)" />
          </div>
          <!-- API key (OpenRouter required, others optional) -->
          <div v-if="showApiKey" class="flex items-center gap-2">
            <label class="text-xs text-slate-500 w-10 shrink-0">
              {{ inferenceProvider === 'openrouter' ? 'Key' : 'Token' }}
            </label>
            <input v-model="providerApiKey" type="password" class="input text-xs flex-1"
        ...
        <!-- Setup hint -->
        <p class="text-xs text-slate-400 mt-2">{{ PROVIDERS.find(p=>p.id===inferenceProvider)?.hint }}</p>
      </div>
    </div>
    ...
    <div v-if="!registryLoaded" class="mb-6">
      <div class="flex items-center justify-between mb-2">
        <div class="h-4 bg-slate-100 rounded w-28 animate-pulse"/>
      </div>
      <div class="grid grid-cols-2 gap-2 mb-4">
        <div v-for="n in 4" :key="n" class="card card-body !py-2.5 !px-3 animate-pulse">
          <div class="h-3 bg-slate-100 rounded w-3/4 mb-1.5"/>
          <div class="h-2.5 bg-slate-100 rounded w-1/2"/>
        </div>
      </div>
    </div>
    <div v-else-if="registry.models.length" class="mb-6">
      <div class="flex items-center justify-between mb-3">
        <div>
          <h2 class="font-semibold text-slate-900">Model Routing</h2>
          <p class="text-xs text-slate-400 mt-0.5">Enable models and see which tasks they handle</p>
        </div>
      ...
      <div class="grid grid-cols-2 gap-2 mb-4">
        <div v-for="m in registry.models" :key="m.filename"
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-1.5">
                ...
                <span v-if="m.enabled" class="badge badge-green text-xs shrink-0">active</span>
              </div>
              <div class="flex flex-wrap gap-1 mt-1">
                <span v-for="cap in m.capabilities" :key="cap"
            ...
            <div class="flex items-center gap-1 shrink-0">
              ...
              <button @click="expandedModel = expandedModel === m.filename ? null : m.filename"
                class="text-xs text-slate-400 hover:text-slate-600 px-1">▾</button>
            </div>
          </div>
          <!-- Score bars — expanded -->
          <div v-if="expandedModel === m.filename"
            class="mt-2 pt-2 border-t border-slate-100 grid grid-cols-2 gap-x-3 gap-y-1">
            <div v-for="(score, task) in m.task_scores" :key="task"
              class="flex items-center gap-1.5">
              <div class="flex-1 h-1 bg-slate-100 rounded-full overflow-hidden">
                <div class="h-full bg-sky-400 rounded-full" :style="`width:${Math.round(score*100)}%`"/>
              </div>
      ...
      <div class="card">
        ...
        <div class="divide-y divide-slate-50">
          <div v-for="row in registry.routing_table" :key="row.task_type"
            ...
            <span v-if="row.model" class="text-sm text-slate-700 font-medium flex-1">
              {{ row.display_name }}
            </span>
            <span v-else class="text-sm text-slate-400 italic flex-1">no model active</span>
            <span v-if="row.model" class="text-xs text-slate-400">
      ...
      <div class="card mt-4">
        <div class="card-header flex items-center justify-between">
          ...
          <button @click="loadHistory" class="text-xs text-slate-400 hover:text-slate-600">↻ Refresh</button>
        </div>
        <div v-if="!routingLog.length" class="card-body text-xs text-slate-400 text-center py-4">
          No routing history yet — run the agent to see which model handles each task.
        </div>
        <div v-else class="divide-y divide-slate-50">
          <div v-for="row in routingLog" :key="row.id"
            ...
            <!-- Arrow -->
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              class="w-3 h-3 text-slate-300 shrink-0"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            <!-- Model -->
            <span class="font-mono text-slate-700 truncate flex-1">{{ row.model }}</span>
            <!-- Result -->
            <span v-if="row.success" class="text-green-600 shrink-0">✓</span>
            <span v-else :class="['shrink-0', row.error_type === 'connection' ? 'text-red-500' : 'text-amber-500']"
              :title="row.summary || row.error_type">✗</span>
            <!-- Duration -->
            <span v-if="row.duration_ms" class="text-slate-400 shrink-0 w-14 text-right tabular-nums">
    ...
    <!-- Recommended models -->
    <h2 class="font-semibold text-slate-900 mb-2">Recommended Models</h2>
    <div class="grid grid-cols-2 gap-2">
      <div v-for="m in recommendedModels" :key="m.name" class="card card-body !py-2.5 !px-3">
        <div class="flex items-center gap-2">
          <!-- Icon -->
          <div class="w-6 h-6 rounded bg-slate-100 flex items-center justify-center shrink-0 overflow-hidden">
            <img :src="modelIconUrl(m.name)" class="w-5 h-5 object-contain"
              @error="(e: Event) => { const t = e.target as HTMLImageElement; if(t) t.style.display='none' }"
            />
          </div>
          <!-- Name + description -->
          <div class="flex-1 min-w-0">
            <div class="font-medium text-xs text-slate-900 truncate">{{ m.name }}</div>
            <div class="text-xs text-slate-400 truncate">{{ m.notes }}</div>
          </div>
          ...
          <div class="flex flex-col items-end gap-1 shrink-0">
            ...
            <button @click="downloadModel(m)" :disabled="!!downloadingModel"
    ...
    <div v-if="downloadingModel" class="mt-3 px-1">
      ...
      <div class="h-1 bg-slate-100 rounded-full overflow-hidden">
        <div class="h-full bg-sky-500 transition-all duration-300 rounded-full" :style="`width: ${downloadPercent}%`"/>
      </div>
    </div>

    <div v-if="downloadSuccess" class="mt-2 flex items-center gap-2 px-1 text-xs text-green-700">
...
function errorTypeLabel(errType: string): string {
...
async function saveHFToken() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      downloadError.value = null
      // Retry the pending download automatically
      if (pendingDownload.value) {
        const m = pendingDownload.value
        pendingDownload.value = null
        setTimeout(() => downloadModel(m), 300)
      }
      setTimeout(() => { tokenSaved.value = false }, 2000)
    }
  } catch (e) {
    downloadError.value = `Could not save token: ${e}`
  } finally {
