<template>
  <div class="p-4 max-w-4xl mx-auto w-full" v-if="app">
    <!-- Header -->
    <div class="flex items-center gap-4 mb-6">
      <RouterLink to="/" class="text-slate-400 hover:text-slate-600 text-sm">← Back</RouterLink>
      <div class="flex items-center gap-3 flex-1">
        <div class="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center overflow-hidden">
          <img :src="`https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/${(app.key||'').replace(/_/g,'-')}.png`"
            :alt="app.display_name" class="w-8 h-8 object-contain"
        ...
        <div>
          <h1 class="text-xl font-semibold text-slate-900">{{ app.display_name }}</h1>
      ...
      <div class="flex gap-2">
        <button v-if="app.status === 'running'" @click="restart" class="btn-secondary btn-sm">Restart</button>
        <button v-if="app.status === 'running'" @click="disable" class="btn-secondary btn-sm text-amber-600">Disable</button>
        <button v-if="app.status === 'disabled'" @click="enable" class="btn-primary btn-sm">Enable</button>
        <button @click="showRemove = true" :disabled="removing" class="btn-secondary btn-sm text-red-500">{{ removing ? 'Removing…' : 'Remove' }}</button>
        <button v-if="app.status === 'running' || app.status === 'disabled'"
          @click="updateApp" :disabled="updating"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="card card-body lg:col-span-1 space-y-4">
        <div>
          ...
          <dl class="space-y-2 text-sm">
            <div class="flex justify-between">
              ...
              <dd class="font-mono text-slate-900">{{ app.host_port ?? '—' }}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-slate-500">Criticality</dt>
              <dd class="capitalize" :class="critColor">{{ app.criticality }}</dd>
            </div>
            <div class="flex justify-between">
              <dt class="text-slate-500">Image</dt>
              <dd class="font-mono text-xs text-slate-600 text-right max-w-32 truncate">{{ app.image }}</dd>
            </div>
            <div class="flex justify-between items-center">
              <dt class="text-slate-500">Version tag</dt>
              <dd class="flex items-center gap-1">
                <span v-if="!showVersionPin" class="font-mono text-xs text-slate-600">{{ (app as any).image_tag || 'latest' }}</span>
                <input v-else v-model="pinnedTag" type="text" placeholder="e.g. 4.0.9"
                  class="input text-xs w-24 py-0.5" />
                <button @click="toggleVersionPin"
        ...
        <div v-if="app.host_port">
          <div class="section-title mb-2">Access</div>
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div v-if="postInstallSteps.length" class="card">
          <div class="card-header flex items-center justify-between">
            ...
            <div v-for="(step, idx) in postInstallSteps" :key="idx"
              ...
              <div class="flex items-start gap-2">
                <button @click="step._done = !step._done"
                  ...
                  <span v-if="step._done" class="text-xs leading-none">✓</span>
                </button>
                <div class="flex-1 min-w-0">
                  <div :class="['text-sm font-medium', step._done ? 'line-through text-slate-400' : 'text-slate-800']">
                    {{ step.title }}
                    <span v-if="step.required" class="ml-1 text-xs text-red-400">required</span>
                  </div>
                  <div class="text-xs text-slate-500 mt-0.5">{{ step.description }}</div>
                </div>
                <a v-if="step.link" :href="step.link" target="_blank" rel="noopener"
                  class="text-xs text-sky-500 hover:text-sky-600 shrink-0">Open ↗</a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-header flex items-center justify-between">
            ...
            <button @click="fetchLogs" class="text-xs text-sky-500 hover:text-sky-600">Refresh</button>
          </div>
        ...
        <div v-if="appConfig && appConfig.schema && appConfig.schema.length" class="card">
          <div class="card-header flex items-center justify-between">
            <span class="font-semibold text-sm">Configuration</span>
            <span v-if="configSaved" class="text-xs text-green-600 font-medium">Saved ✓</span>
          </div>
          <div class="card-body space-y-4">
            <div v-for="field in appConfig.schema" :key="field.key">
              <!-- Providers list (DDNS Updater style) -->
              <template v-if="field.type === 'providers_list'">
                <label class="label">{{ field.label }}</label>
                <div class="space-y-2">
                  <div v-for="(provider, idx) in (configValues.providers || [])" :key="idx"
                    class="flex gap-2 items-center rounded-lg border border-slate-200 px-3 py-2">
                    <input v-model="provider.provider" type="text" placeholder="cloudflare"
                      class="input text-xs flex-1" @input="configDirty = true" />
                    <input v-model="provider.host" type="text" placeholder="@ or subdomain"
                      class="input text-xs w-28" @input="configDirty = true" />
                    <input v-model="provider.token" type="password" placeholder="API token"
                      class="input text-xs w-36" @input="configDirty = true" />
                    <button @click="removeProvider(idx)" class="text-slate-300 hover:text-red-400 shrink-0">✕</button>
                  </div>
                  <button @click="addProvider"
                    class="text-xs text-sky-500 hover:text-sky-600 font-medium">+ Add provider</button>
                </div>
              ...
              <template v-else-if="field.type === 'select'">
                <label class="label">{{ field.label }}</label>
                <select v-model="configValues[field.key]" class="input" @change="configDirty = true">
              ...
              <template v-else>
                ...
                <input v-model="configValues[field.key]"
                  ...
                  class="input" @input="configDirty = true" />
                <p v-if="field.help" class="text-xs text-slate-400 mt-1">{{ field.help }}</p>
              </template>
            </div>

            <div class="flex gap-2 pt-2 border-t border-slate-100">
              <button @click="saveConfig" :disabled="!configDirty || savingConfig"
                class="btn-primary btn-sm">
                {{ savingConfig ? 'Saving…' : 'Save configuration' }}
              </button>
              <button @click="applyConfig" :disabled="!configSaved"
    ...
    <!-- Remove modal -->
    <Teleport to="body">
      <div v-if="showRemove" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/30 backdrop-blur-sm" @click="showRemove = false"/>
        <div class="relative card w-full max-w-sm mx-4 card-body">
          <h3 class="font-semibold text-slate-900">Remove {{ app.display_name }}?</h3>
          <p class="text-sm text-slate-500 mt-2">The container will be stopped and removed. Wiring to other apps will be cleaned up.</p>
          <label class="flex items-center gap-2 mt-3 cursor-pointer">
            <input v-model="deleteConfig" type="checkbox" class="rounded border-slate-300" />
            <span class="text-sm text-slate-700">Also delete config folder</span>
          </label>
          <div class="flex gap-3 mt-4">
            <button @click="showRemove = false" class="btn-secondary flex-1">Cancel</button>
            <button @click="doRemove" class="btn-danger flex-1">Remove</button>
          </div>
        </div>
      </div>
    </Teleport>
  </div>
  <div v-else class="p-6 text-center text-slate-400">Loading…</div>
</template>
...
const showPuidOverride = ref(false)
const puidValue = ref(1000)
const pgidValue = ref(1000)
...
async function savePin() {
  ...
  try {
    const res = await fetch(`/api/v1/apps/${app.value?.key}/pin-tag`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ image_tag: pinnedTag.value || 'latest' }),
    })
    ...
    toast.error('Could not save pin.', String(e))
...
async function checkUpdate() {
  ...
  try {
    const res = await fetch(`/api/v1/apps/${app.value?.key}/check-update`)
...
async function startUpdate() {
  updating.value = true
  try {
    const res = await fetch(`/api/v1/apps/${app.value?.key}/update`, { method: 'POST' })
    const data = await res.json()
    if (data.ok) toast.success(data.message)
    else toast.error('Update failed.', data.detail)
  } catch (e) {
    toast.error('Update failed.', String(e))
