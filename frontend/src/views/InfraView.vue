<template>
  <div class="p-4 max-w-4xl mx-auto w-full">
    <div class="mb-4">
      ...
      <p class="page-subtitle">Auth · Tunnel · Dashboard · Management · VPN</p>
    </div>
...
<span class="text-base">🔀</span>
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-xs text-slate-400 mt-0.5" v-if="traefikStatus">
        ...
        <div class="flex items-center gap-3 shrink-0">
          ...
          <span v-if="traefikStatus?.version" class="text-xs text-slate-400 font-mono">
    ...
    <div class="grid grid-cols-5 gap-2 mb-6">
      <button
        v-for="slot in SLOT_ORDER" :key="slot"
        ...
        <div class="text-xs font-medium text-slate-700 capitalize mt-1">{{ slot }}</div>
    ...
    <div v-if="activeSlot" class="card mb-6">
      <div class="card-header flex items-center justify-between">
        ...
        <button @click="activeSlot = null" class="text-slate-400 hover:text-slate-600">✕</button>
      </div>
      <div class="card-body space-y-3">

        <!-- Current provider info — single-provider slots -->
        <div v-if="activeSlot !== 'tunnel' && slotMap[activeSlot]?.status === 'active'" class="flex items-center gap-3 p-3 rounded-lg bg-green-50 border border-green-100">
          <div class="w-8 h-8 flex items-center justify-center overflow-hidden">
            <img :src="providerIconUrl(slotMap[activeSlot]?.provider ?? '')"
          ...
          <div class="flex-1">
            <div class="text-sm font-medium text-green-800">{{ slotMap[activeSlot]?.display_name || slotMap[activeSlot]?.provider }}</div>
            <div v-if="slotMap[activeSlot]?.deployed_at" class="text-xs text-green-600">
              Since {{ new Date((slotMap[activeSlot]?.deployed_at ?? 0) * 1000).toLocaleDateString() }}
            </div>
          </div>
          <div class="flex gap-2">
            <button @click="verify(activeSlot!)" :disabled="verifying === activeSlot" class="btn-secondary btn-sm">
              {{ verifying === activeSlot ? '…' : 'Verify' }}
            </button>
            <button @click="openSwap(activeSlot!)" class="btn-secondary btn-sm">Swap</button>
          </div>
        </div>

        <!-- Tunnel: multi-provider — show all active tunnels -->
        <div v-if="activeSlot === 'tunnel'" class="space-y-3">
          <!-- Active tunnels list -->
          <div v-if="activeTunnelProviders.length" class="space-y-2">
            <div class="text-xs font-medium text-slate-500 uppercase tracking-wider">Active tunnels</div>
            <div v-for="tp in activeTunnelProviders" :key="tp.provider"
              class="flex items-center gap-3 p-3 rounded-lg bg-green-50 border border-green-100">
              <div class="w-8 h-8 flex items-center justify-center overflow-hidden">
                <img :src="providerIconUrl(tp.provider)"
                  class="w-7 h-7 object-contain"
                  @error="(e: Event) => { const t = e.target as HTMLImageElement; if(t){t.style.display='none'; const s=t.nextElementSibling as HTMLElement; if(s) s.style.display='block'} }"
                />
              ...
              <div class="flex-1">
                <div class="text-sm font-medium text-green-800 capitalize">{{ tp.provider }}</div>
                <div v-if="tp.deployed_at" class="text-xs text-green-600">
                  Active since {{ new Date((tp.deployed_at ?? 0) * 1000).toLocaleDateString() }}
                </div>
              </div>
              <div class="flex gap-2">
                <button @click="verifyTunnel(tp.provider)" :disabled="verifying === tp.provider"
                  class="btn-secondary btn-sm text-xs">
                  {{ verifying === tp.provider ? '…' : 'Verify' }}
                </button>
                <button @click="removeTunnel(tp.provider)" :disabled="removingTunnel === tp.provider"
          ...
          <!-- Add tunnel prompt -->
          <div v-if="filteredAvailableProviders.length" class="pt-1">
          ...
          <div v-else-if="activeTunnelProviders.length"
        ...
        <!-- Verify result -->
        <div v-if="verifyResults[activeSlot]" :class="['text-xs rounded-lg px-3 py-2', verifyResults[activeSlot].ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600']">
          ...
          <div class="grid grid-cols-4 gap-2">
            <button
              ...
              <div class="w-8 h-8 mb-1 flex items-center justify-center overflow-hidden">
                <img :src="providerIconUrl(p.key)" class="w-7 h-7 object-contain"
                  @error="(e: Event) => { const t = e.target as HTMLImageElement; if(t){t.style.display='none'; const s=t.nextElementSibling as HTMLElement; if(s) s.style.display='block'} }"
                />
                <span class="text-xl hidden">{{ PROVIDER_ICONS[p.key] ?? '⚙️' }}</span>
              </div>
              <div class="text-xs font-medium text-slate-800 leading-tight">{{ p.display_name }}</div>
            </button>
          </div>
        </div>
        ...
        <template v-if="selectedProvider && (activeSlot === 'tunnel' || slotMap[activeSlot]?.status !== 'active' || showingSwap)">
          <div v-for="field in selectedProvider.fields" :key="field.key">
            <template v-if="field.type === 'info'">
              <div class="rounded-lg bg-sky-50 border border-sky-100 p-3 text-sm text-sky-800">{{ field.help }}</div>
            </template>
            <template v-else-if="field.type === 'checkbox'">
              <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" v-model="deployConfig[field.key]" class="rounded border-slate-300"/>
                <div>
            ...
            <template v-else>
              <label class="label">{{ field.label }}<span v-if="field.required" class="text-red-400 ml-0.5">*</span></label>
              <input v-model="deployConfig[field.key]"
          ...
          <div v-if="deployError" class="rounded-lg bg-red-50 text-red-700 text-sm p-3">{{ deployError }}</div>
          <div v-if="deploySuccess" class="rounded-lg bg-green-50 text-green-700 text-sm p-3">{{ deploySuccess }}</div>

          <div class="flex gap-3">
            <button @click="showingSwap = false; selectedProvider = null; deployConfig = {}" class="btn-secondary flex-1">Cancel</button>
            <button @click="deploy" :disabled="deploying || !selectedProvider" class="btn-primary flex-1">
        ...
        <!-- Empty slot prompt -->
        <div v-if="activeSlot !== 'tunnel' && slotMap[activeSlot]?.status !== 'active' && !availableProviders.length" class="text-sm text-slate-400 text-center py-4">
    ...
    <div>
      <h2 class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">All providers</h2>
      <div class="space-y-3">
        <div v-for="(schemas, slotName) in schemasBySlot" :key="slotName">
          <div class="text-xs font-medium text-slate-500 capitalize mb-1.5">{{ slotName }}</div>
          <div class="grid grid-cols-4 gap-1.5">
            <div v-for="p in schemas" :key="p.key"
              ...
              <div class="w-6 h-6 flex items-center justify-center overflow-hidden shrink-0">
                <img :src="providerIconUrl(p.key)" class="w-5 h-5 object-contain"
                  @error="(e: Event) => { const t = e.target as HTMLImageElement; if(t){t.style.display='none'; const s=t.nextElementSibling as HTMLElement; if(s) s.style.display='block'} }"
                />
