...
The technical runtime uses `mediastack` as its package name (systemd unit: `mediastack.service`, install dir: `/opt/mediastack`). This is normal — the marketing name and the package name are separate things.
...
After install, the URL, login info, and operator commands are written to `/opt/mediastack/POST_INSTALL.txt`.
...
After install, `mediastack` is available at `/opt/mediastack/bin/mediastack`:
...
sudo /opt/mediastack/bin/mediastack uninstall --yes

# Purge (removes everything including data and managed containers)
sudo /opt/mediastack/bin/mediastack purge --yes
```

> **Warning:** `mediastack purge` removes every Docker container labeled `mediastack.managed=true`. Do not apply this label to containers you manage outside of S.L.O.P.
