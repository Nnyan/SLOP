...
"""
import sys, json, os, pathlib, re, time, subprocess, select, tty, termios
...
def main() -> None:
    ...
    DIM   = "\033[2m";    GREEN = "\033[0;32m"; YELLOW = "\033[1;33m"
    NOW   = time.time()
    DAY   = 86400
    ...
    def _is_full_run(r: dict) -> bool:
        ...
        return any(k.startswith("B.") and (v.get("pass",0)+v.get("fail",0)) > 0
                   for k,v in s.items())
    ...
    static_failed = False
    try:
        last_static = json.loads((REPO / "data" / "last_test_run.json").read_text())
    ...
    def marker_age(name: str) -> float:
        ...
        try: return (NOW - p.stat().st_mtime) / DAY
        except: return 999.0
    ...
    if full_reasons:
        cmds.append(("test", [PY, str(REPO / "ms-test.py")],
                     "; ".join(full_reasons[:2])))

    # 2. Trend (runs after full test so it shows fresh data)
    curr_rate = (last_full_passes /
                 (last_full_passes + last_full_fails)
                 if (last_full_passes + last_full_fails) > 0 else 1.0)
    trend_reasons: list[str] = []
    if len(full_runs) >= 5 and curr_rate < prev_pass_rate - 0.05:
        trend_reasons.append(
            f"pass rate dropped {prev_pass_rate:.0%}→{curr_rate:.0%}")
    if len(runs) >= 10 and marker_age("trend") >= 7:
        trend_reasons.append(f"{len(runs)} runs recorded")

    if trend_reasons:
        cmds.append(("trend", [PY, str(REPO / "ms-test.py"), "--trend"],
                     "; ".join(trend_reasons)))
    ...
    if si_reasons:
        cmds.append(("self-improve",
                     [PY, str(REPO / "ms-test.py"), "--self-improve"],
                     si_reasons[0]))

    # Check if audit should run (significant structural changes)
    audit_worthy = changed(r"backend/api/", r"frontend/src/views/", r"catalog/apps/",
                           r"backend/core/schema", r"backend/infra/providers/")
    if audit_worthy and not FULL_MODE:
        snap_path = REPO / ".ms-audit-snapshot.json"
        if not snap_path.exists():
            cmds.append(("audit", [PY, str(REPO / "ms-audit")],
                         "first audit run — no previous snapshot"))
        else:
            cmds.append(("audit", [PY, str(REPO / "ms-audit")],
                         "API/frontend/manifest changes detected"))
