...
def build_schema_from_migrations() -> str:
    ...
    for row in rows:
        ...
        # sqlite_master stores CREATE TABLE without IF NOT EXISTS.
        # Add it back so schema.sql matches what existing tests and tools
        # expect (they grep for "CREATE TABLE IF NOT EXISTS <name>").
        ...
        # Remove spurious double-quotes around table names that SQLite adds
        # when a table is created via ALTER TABLE ... RENAME (e.g. apps after 002).
        sql = re.sub(r'CREATE TABLE IF NOT EXISTS "(\w+)"', r'CREATE TABLE IF NOT EXISTS \1', sql)
