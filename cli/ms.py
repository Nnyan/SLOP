#!/usr/bin/env python3
"""ms — Mediastack command-line interface.
...
Config:
  API URL: set MEDIASTACK_URL env var or --url flag (default http://localhost:8080)
  Output:  set MEDIASTACK_NO_COLOR=1 to disable ANSI colors
...
NO_COLOR = os.environ.get("MEDIASTACK_NO_COLOR") or not sys.stdout.isatty()
...
class APIClient:
    ...
    def _req(self, method: str, path: str, body: dict | None = None) -> Any:
        ...
        try:
            ...
            raise APIError(
                f"Cannot reach Mediastack at {self.base}.\n"
                f"  Is the server running? ({e.reason})\n"
                f"  Set MEDIASTACK_URL or use --url to override."
            ) from e
        ...
        while time.monotonic() < deadline:
            ...
            if progress.get("done"):
                if progress.get("ok"):
                    ...
                    return 1
                return 0
        _err(f"Install timed out after 10 minutes. Check server: ms apps list")
        return 1
    else:
...
def cmd_wizard(api: APIClient, _args: argparse.Namespace) -> int:
    print()
    print(bold("  Mediastack Setup Wizard"))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="URL",
        help="Mediastack API base URL (default: $MEDIASTACK_URL or http://localhost:8080)",
    )
    ...
    try:
        if args.command == "status":
            ...
            if args.apps_command == "list":
                return cmd_apps_list(api, args)
            elif args.apps_command == "install":
                return cmd_apps_install(api, args)
            elif args.apps_command == "remove":
                return cmd_apps_remove(api, args)
            elif args.apps_command == "disable":
                return cmd_apps_disable(api, args)
            elif args.apps_command == "enable":
                return cmd_apps_enable(api, args)
            elif args.apps_command == "restart":
                return cmd_apps_restart(api, args)
            elif args.apps_command == "logs":
                return cmd_apps_logs(api, args)
            else:
