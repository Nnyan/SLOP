...
SQLite migration runner for Mediastack v4.
...
def _apply_python_migration(conn: sqlite3.Connection, mig: _MigrationFile) -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        f"_mediastack_migration_{mig.version:03d}",
        str(mig.path),
    )
    if spec is None or spec.loader is None:
        raise MigrationError(
            f"Could not load migration module {mig.filename}. "
...
def _stamp_baseline(conn: sqlite3.Connection, baseline: _MigrationFile) -> None:
    ...
    log.info(
        ...
        baseline.version, baseline.filename,
    )
...
def _apply_one_migration(
    mig: "_MigrationFile", db_path: Path, result: MigrationResult,
) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        restore_cmd = (
            f"cp {result.backup_path} {db_path}" if result.backup_path else
            "no backup taken (backup=False)"
        )
        raise MigrationError(
