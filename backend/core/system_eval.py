...
LLM_TIERS: list[tuple[int, str, list[str]]] = [
    ...
    (5500, "llama3.1:8b", ["smollm2:1.7b", "llama3.2:3b", "qwen2.5:3b", "phi4-mini", "llama3.1:8b"]),
...
def estimate_stack_ram(app_keys: list[str]) -> int:
    ...
    total = MEDIASTACK_OVERHEAD_MB + OS_BASELINE_MB
...
def recommend_llm(headroom_mb: int,
                     ram_bandwidth_gbs: float = 0.0) -> tuple[str, list[str], str | None]:
    ...
    for min_mb, tier, models in LLM_TIERS:
        if headroom_mb >= min_mb:
            ...
            if tier == "smollm2":
                warning = (
                    f"Only SmolLM2 1.7B fits safely in available RAM. "
                    f"Diagnostic quality will be limited — acceptable for triage."
                )
...
def detect_gpu() -> dict[str, Any]:
    ...
    result: dict[str, str | int | bool | None] = {
        "vendor": None, "name": None, "vram_mb": 0,
        "cuda_version": None, "inference_capable": False,
    }
    try:
        out = _sp.run(
        ...
        if lspci.returncode == 0:
            for line in lspci.stdout.splitlines():
                if "VGA" in line or "3D" in line:
                    ...
                    if "Intel" in line: result["vendor"] = "intel"
                    elif "AMD" in line or "ATI" in line: result["vendor"] = "amd"
                    elif "NVIDIA" in line: result["vendor"] = "nvidia"
                    break
    except Exception:
        pass
    return result
...
def detect_os() -> dict[str, Any]:
    ...
    result = {
        "distro": "", "version": "", "arch": platform.machine(),
...
def recommend_llm_with_gpu(headroom_mb: int, gpu: dict[str, Any],
                               ram_bandwidth_gbs: float = 0.0) -> tuple[str, list[str], str | None]:
...
def benchmark_ram_bandwidth_gb() -> float:
    ...
    import time as _t, array as _arr
...
def benchmark_storage_mb_s(path: str = "/tmp") -> float:
    """Sequential write speed in MB/s (16 MB test file)."""
    import time as _t, os as _os, tempfile as _tf
...
def _detect_gpu_rocm() -> dict[str, Any] | None:
    ...
    try:
        import subprocess as _sp
        r = _sp.run(["rocm-smi", "--showmeminfo", "vram", "--json"],
                    capture_output=True, text=True, timeout=5)
    except Exception:
        return None
    if r.returncode != 0:
...
def _detect_gpu_apple_metal() -> dict[str, Any] | None:
    ...
    try:
        import subprocess as _sp
        r = _sp.run(["system_profiler", "SPDisplaysDataType", "-json"],
                    capture_output=True, text=True, timeout=5)
    except Exception:
        return None
    if r.returncode != 0:
...
def get_cached_profile(
    ...
    if (not force
            ...
            and (now - _profile_cache_at) < _PROFILE_CACHE_TTL):
        log.debug("system_eval: returning cached profile (%.0fs old)",
                  now - _profile_cache_at)
...
def evaluate_model_compatibility(
    model_size_gb: float, quantization: str,
    system_ram_gb: float, available_ram_gb: float,
    cpu_cores: int, avx2: bool, gpu: dict[str, Any], storage_free_gb: float,
) -> dict[str, Any]:
    ...
    if available_ram_gb < model_size_gb:
        issues.append(f"Insufficient RAM: need ~{ram_needed:.1f} GB, have {available_ram_gb:.1f} GB")
    elif available_ram_gb < ram_needed:
        warnings.append(f"RAM tight: {available_ram_gb:.1f} GB available, need ~{ram_needed:.1f} GB")
    if not avx2:
        issues.append("CPU lacks AVX2 — required by llama.cpp for CPU inference")
    if storage_free_gb < model_size_gb:
        issues.append(f"Insufficient storage: need {model_size_gb:.1f} GB, have {storage_free_gb:.1f} GB")
    ...
    return {
        ...
        "estimated_tokens_per_second": tps, "inference_mode": mode,
    }
