...
def _action_restart_container(app_key: str, _detail: str) -> dict[str, Any]:
    ...
    return {"executed": False, "requires_approval": False,
...
def _action_reload_config(app_key: str, _detail: str) -> dict[str, Any]:
    ...
    return {"executed": True, "requires_approval": False,
...
def _action_pull_image(app_key: str, _detail: str) -> dict[str, Any]:
    ...
    if not (app and app.image):
        return {"executed": False, "requires_approval": True,
...
def _action_rewire(app_key: str, _detail: str) -> dict[str, Any]:
    try:
        ...
        with StateDB() as db:
            ...
            for row in stale:
                db.execute("UPDATE wiring SET status='pending' WHERE id=?",
                           (row["id"],))
        n = len(stale)
        return {
            "executed": True, "requires_approval": False,
            "message": (
...
def _action_restart_managed_service(app_key: str,
                                    _detail: str) -> dict[str, Any]:
...
def _action_remount_storage(_app_key: str, _detail: str) -> dict[str, Any]:
    ...
    with StateDB() as db:
        bad_stores = db.execute(
            "SELECT name, source_type, mount_point FROM storage_sources "
            "WHERE status='error'",
        ).fetchall()
    results = []
    for s in bad_stores:
...
def _action_reprovision_hostname(app_key: str,
                                 _detail: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "message": (
                f"Manual: re-register hostname for '{app_key}' "
                f"with your tunnel. ({_re})"
            ),
        }
...
async def execute_action(action_type: str, app_key: str,
    ...
    log.warning(
        "AI auto-act: executing '%s' for '%s' — "
        "user has opted into Tier 2 for this action type.",
        action_type, app_key,
    )
    ...
    if handler is None:
        return {"executed": False, "requires_approval": True,
                "message": (
                    f"Action '{action_type}' is not implemented — "
                    "requires manual intervention."
                )}
    try:
        return handler(app_key, detail)
    except Exception as e:
        log.error("AI auto-act failed for '%s'/'%s': %s",
                  action_type, app_key, e)
        return {"executed": False, "requires_approval": False,
                "message": str(e)}
