...
def validate_gguf(path: Path) -> GGUFValidationResult:
    ...
    if not path.exists():
        return GGUFValidationResult(
            valid=False, path=path, file_size_mb=0, gguf_version=None,
    ...
    if size_bytes < MIN_FILE_SIZE_MB * 1_048_576:
        return GGUFValidationResult(
            valid=False, path=path, file_size_mb=size_mb, gguf_version=None,
            error=(
    ...
    try:
        with open(path, "rb") as f:
            ...
            if version not in GGUF_SUPPORTED_VERSIONS:
                return GGUFValidationResult(
                    valid=False, path=path, file_size_mb=size_mb, gguf_version=version,
                    error=(
    ...
    log.info(
        ...
        path.name, size_mb, version, tensor_count,
    )
...
def resolve_gguf_url(url_or_hf: str) -> str:
    ...
    if "huggingface.co" in url_or_hf and "/blob/" in url_or_hf:
        return url_or_hf.replace("/blob/", "/resolve/")
...
def download_gguf(
    ...
    try:
        ...
        yield DownloadProgress(url=url_or_hf, bytes_downloaded=0, total_bytes=None,
                               percent=None, done=True, error=str(e))
        return GGUFValidationResult(
    ...
    try:
        ...
        msg = (
            f"HTTP {e.code} downloading from {url}. "
        ...
        if dest_path.exists():
            dest_path.unlink()
        yield DownloadProgress(url=url, bytes_downloaded=0, total_bytes=None,
                               percent=None, done=True, error=msg)
        return GGUFValidationResult(
...
RECOMMENDED_MODELS = [
    {
        ...
        "size_gb": 2.4,
        "recommended_for": "mediastack-agent",
