...
def best_model_for(task_type: str) -> ModelRecord | None:
    ...
    log.debug(
        ...
        task_type, winner.filename, winner.score_for(task_type),
    )
    return winner
...
def sync_registry_with_files(filenames: list[str]) -> None:
    ...
    with StateDB() as db:
        existing = {
            r["filename"]
            for r in db.execute("SELECT filename FROM llm_model_registry").fetchall()
        }
        ...
        if not existing:
            ...
            db.execute(
                """INSERT INTO llm_model_registry
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (filename, display_name, int(enabled or 0),
                 json.dumps(caps), json.dumps(scores),
                 priority or 5, context_window or 4096,
                 ollama_name, notes),
            )
        else:
            updates: dict[str, Any] = {"updated_at": int(_t.time())}
            if enabled is not None:       updates["enabled"] = int(enabled)
            if display_name is not None:  updates["display_name"] = display_name
            if capabilities is not None:  updates["capabilities"] = json.dumps(capabilities)
            if task_scores is not None:   updates["task_scores"] = json.dumps(task_scores)
            if priority is not None:      updates["priority"] = priority
            if context_window is not None: updates["context_window"] = context_window
            if ollama_name is not None:   updates["ollama_name"] = ollama_name
            if notes is not None:         updates["notes"] = notes
            cols = ", ".join(f"{k} = ?" for k in updates)
            db.execute(
                f"UPDATE llm_model_registry SET {cols} WHERE filename = ?",
