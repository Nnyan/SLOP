...
KNOWLEDGE_BASE: list[dict[str, str]] = [
    # Docker errors
    {"id": "docker_oom", "title": "Container killed: OOM", "text":
     "Container killed with exit code 137 means Out of Memory. "
    ...
    {"id": "docker_permission", "title": "Permission denied in container", "text":
     "Permission denied errors in containers usually mean PUID/PGID mismatch. "
    ...
    {"id": "docker_port_conflict", "title": "Port already in use", "text":
     "Error 'address already in use' or 'port is already allocated' means another process "
    ...
    {"id": "docker_network", "title": "Container not on network", "text":
     "Container can't reach other containers: check they're on the same Docker network. "
     "All Mediastack apps should be on the 'mediastack' network. "
     "Fix: docker network inspect mediastack — verify the container appears in Containers list."},

    # Database errors
    {"id": "db_locked", "title": "SQLite database is locked", "text":
     "Database locked error means two processes are trying to write simultaneously, "
    ...
    {"id": "db_corrupt", "title": "Database corruption", "text":
     "Database corruption usually happens from sudden power loss or disk full. "
    ...
    # Traefik errors
    {"id": "traefik_cert", "title": "Traefik certificate not issued", "text":
     "Certificate not being issued: check CF_DNS_API_TOKEN is set and valid. "
    ...
    {"id": "traefik_middleware", "title": "Traefik middleware not found", "text":
     "Error 'middleware X@docker does not exist': the middleware is defined via labels "
    ...
    {"id": "traefik_docker_api", "title": "Traefik Docker API version mismatch", "text":
     "Error 'client version 1.24 is too old': Traefik v3.3 has a known incompatibility "
     "with Docker 29+. Fix: use traefik:v3.2 or traefik:latest image tag. "
     "Update the compose fragment and recreate the container."},

    # Sonarr/Radarr
    {"id": "arr_indexer", "title": "Sonarr/Radarr indexer errors", "text":
     "No indexers configured: install Prowlarr first, then sync it to Sonarr/Radarr "
    ...
    {"id": "arr_download", "title": "Sonarr/Radarr download client errors", "text":
     "Download client not configured or unreachable: configure qBittorrent or SABnzbd "
    ...
    {"id": "arr_quality", "title": "No downloads found by arr apps", "text":
     "Sonarr/Radarr not downloading: check quality profiles allow the available formats. "
    ...
    # Plex/Jellyfin
    {"id": "media_transcode", "title": "Plex/Jellyfin transcoding issues", "text":
     "Transcoding failing or slow: check hardware transcoding is enabled if available. "
    ...
    {"id": "plex_claim", "title": "Plex not signing in", "text":
     "Plex claim token expired or missing: get a fresh token from plex.tv/claim "
    ...
    # Storage/disk
    {"id": "disk_full", "title": "Disk full", "text":
     "No space left on device: check disk usage with 'df -h'. "
    ...
    # General container
    {"id": "container_restart", "title": "Container restart loop", "text":
     "Container in restart loop (status: restarting): check logs immediately after restart "
    ...
    {"id": "container_unhealthy", "title": "Container health check failing", "text":
     "Container shows as 'unhealthy': the health check command is failing. "
...
class ChromaRetriever:
    ...
    def build(self, docs: list[dict[str, str]]) -> None:
        ...
        self._collection = client.get_or_create_collection(
            "mediastack_kb", embedding_function=emb_fn
        )
        existing = set(self._collection.get()["ids"])
        to_add = [d for d in docs if d["id"] not in existing]
        if to_add:
