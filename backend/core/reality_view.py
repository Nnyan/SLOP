...
process environment).  In reconciler-trust terms (Stream E vocabulary) this is
**GROUND** data: every field is observed from the running process and a green
value can go red against physics — it is never text-vs-text (XREF).
...
PINNED schema (Stream A owns; Stream B consumes verbatim)::
...
contract renegotiation with Stream B.
...
def observe_install_dir_owner(
    ...
    *,
    ...
    except (KeyError, Exception):  # noqa: BLE001 - any lookup failure → numeric
        return str(uid)
...
def observe_env_sources(
    ...
    return {
        name: classify_env_source(name, environ, dotenv_values)
...
def reality_var_names() -> list[str]:
    ...
    The SLOP-managed wizard vars (``_SLOP_MANAGED_VARS``) plus the
    deploy-relevant operator vars (the ``MS_*`` knobs that ``core/config.py``
    reads, the Rocinante failure surface).  Imported lazily and defensively so
    this RUNTIME module never hard-depends on the API layer at import time.
    """
    managed: frozenset[str] = frozenset()
    try:
        from backend.api.apps import _SLOP_MANAGED_VARS

        managed = _SLOP_MANAGED_VARS
    except Exception:  # noqa: BLE001 - never let provenance break the view
        managed = frozenset()
    operator = (
        "MS_TRUSTED_HOSTS",
        "MS_BIND_HOST",
    ...
    return sorted(set(managed) | set(operator))
...
def assemble_live_reality_view() -> dict[str, Any]:
    ...
    except Exception:  # noqa: BLE001 - the view must never break health
        return {
            "schema_version": REALITY_SCHEMA_VERSION,
