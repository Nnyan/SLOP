...
class StateDB:
    ...
    def update_platform(self, **kwargs: Any) -> None:
        ...
        self._c.execute(
            f"UPDATE platform SET {cols} WHERE id = 1",  # noqa: S608  # nosec B608  # cols built from hardcoded allowed-set, not user input
            list(updates.values()),
        )
        self._c.commit()
    ...
    def update_slot(self, slot: str, **kwargs: Any) -> None:
        ...
        cols = ", ".join(f"{k} = ?" for k in updates)
        self._c.execute(
            f"UPDATE infra_slots SET {cols} WHERE slot = ?",  # noqa: S608  # nosec B608  # cols built from hardcoded allowed-set, not user input
    ...
    def upsert_tunnel_provider(self, provider: str, **kwargs: Any) -> None:
        ...
        if existing:
            ...
            cols = ", ".join(f"{k} = ?" for k in updates)
            self._c.execute(
                f"UPDATE infra_tunnel_providers SET {cols} WHERE provider = ?",  # noqa: S608  # nosec B608  # cols built from hardcoded allowed-set, not user input
    ...
    def upsert_app(self, key: str, **kwargs: Any) -> int:
        ...
        if existing:
            ...
            if updates:
                ...
                cols = ", ".join(f"{k} = ?" for k in updates)
                self._c.execute(
                    f"UPDATE apps SET {cols} WHERE key = ?",  # noqa: S608  # nosec B608  # cols built from hardcoded allowed-set, not user input
    ...
    # ── Async jobs (wizard / Ollama setup) ────────────────────────────────
    #
    # Persists the background-job state that used to live only in the
    # _wizard_jobs / _ollama_jobs in-memory dicts. The running thread still
    # mutates the in-memory dict (hot path); every mutation is mirrored here
    # so a backend restart does not orphan a polling client. `payload` is the
    # full job dict serialised as JSON — the exact shape the polling endpoint
    # returns, kept opaque so the HTTP API can evolve without a migration.
