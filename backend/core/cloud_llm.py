...
_SECRET_PATTERNS = [
    re.compile(r'(cf[a-z_]*token|api[_-]?key|password|secret|token|bearer)\s*[=:]\s*([^\s\n\'"]{8,})',
               re.IGNORECASE),
...
def _get_monthly_spend(provider_key: str | None = None) -> float:
    ...
    try:
        ...
        with StateDB() as db:
            if provider_key:
                row = db.execute(
                    "SELECT SUM(cost_usd) FROM cloud_llm_usage "
...
def _record_usage(provider_key: str, model: str, prompt_tokens: int,
                  completion_tokens: int, cost: float,
                  purpose: str = "", app_key: str = "") -> None:
        ...
        with StateDB() as db:
            db.execute(
                """INSERT INTO cloud_llm_usage
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?)""",
                (provider_key, model, prompt_tokens, completion_tokens,
                 prompt_tokens + completion_tokens, cost, purpose, app_key,
                 int(time.time()))
            )
...
async def escalate_to_cloud(
    ...
    for provider_key in cascade:
        result = await _try_one_provider(
            provider_key, sanitized_prompt, subst, prompt,
            purpose, app_key, user_confirmed,
