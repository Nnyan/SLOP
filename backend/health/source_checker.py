...
_NAMED_REGISTRIES: frozenset[str] = frozenset({
    "ghcr.io", "lscr.io", "quay.io", "gcr.io",
})
...
async def _retry_with_bearer_challenge(
    www_auth: str, url: str, headers: dict[str, str],
    ...
    if r2.status_code == 200:
        return "ok", 200, ""
    return ("missing" if r2.status_code == 404 else "unreachable",
            r2.status_code, "")
    ...
    try:
        ...
        if r.status_code == 401:
            chal = await _retry_with_bearer_challenge(
                r.headers.get("www-authenticate", ""), url, headers,
            )
            if chal is not None:
                return chal
...
async def run_source_scan() -> dict[str, Any]:
        ...
        async with sem:
            ...
            if status != "ok":
                log.warning(
                    ...
                    status.upper(), resource_key, source_type,
                    code or "", error,
                )
    ...
    log.info(
        ...
        results["checked"], results["ok"], results["missing"], results["unreachable"],
    )
    return results
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            if provider == "ollama":
                ...
                model = (rec.ollama_name or rec.filename.replace(".gguf", "")) if rec else "phi4-mini"
                ...
                resp = await client.post(call_url, headers=hdrs, json={
                    "model": model,
