...
def detect_anomalies(lookback_hours: int = 168) -> list[AnomalyPattern]:
    ...
    for (app_key, check_name), events in groups.items():
        pat = _pattern_for_group(
            app_key, check_name, events, mw_index, lookback_hours,
        )
        if pat is not None:
            patterns.append(pat)
    ...
    try:
        with StateDB() as db:
            history_exists = db.execute(
                ...
                "WHERE type='table' AND name='health_check_history'"
            ).fetchall()
...
def _describe_typical_time(typical_hour: int | None,
                           typical_day: int | None) -> str:
...
def _pattern_for_group(
    ...
    events = [
        e for e in events
        if not _is_in_maintenance(e["timestamp"], app_key, check_name, mw_index)
    ]
    n = len(events)
    if n < 3:
        return None
    ...
    days = [datetime.fromtimestamp(t, tz=timezone.utc).weekday()
            for t in timestamps]
    ...
    description = (
        f"{check_name} has failed {n} times in the last "
        f"{lookback_hours // 24} days{time_desc}."
    )
    has_window = bool(mw_index.get((app_key, check_name)))
    is_scheduled = (typical_hour is not None or typical_day is not None) \
        and has_window
