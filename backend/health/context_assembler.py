...
def assemble_context(app_key: str, check_name: str,
...
def _section_operation_history(db: Any, app_key: str, cutoff: int, lines: list[str]) -> None:
    try:
        ...
        if ops:
            ...
            for op in ops:
                ts = datetime.fromtimestamp(op["started_at"], tz=timezone.utc).strftime("%m-%d %H:%M")
...
def _section_check_type(db: Any, app_key: str, check_name: str, lines: list[str]) -> None:
    try:
        ...
        if check_name in ("tcp_reachable", "tcp_check") or (check_meta and "tcp" in str(check_meta)):
            lines.append(
                f"Check type: TCP port reachability — failure means port not open, "
                f"not an application logic error. Check docker logs for startup failure."
            )
...
def _section_health_history(db: Any, app_key: str, check_name: str,
                            cutoff: int, lines: list[str]) -> None:
    try:
        hist = db.execute(
        ...
        if hist:
            ...
            hours = [datetime.fromtimestamp(h["checked_at"], tz=timezone.utc).hour
                     for h in failures]
...
def _section_failure_duration(db: Any, app_key: str, check_name: str,
                              lines: list[str]) -> None:
    try:
        _last_ok = db.execute(
        ...
        if _last_ok and _last_ok[0]:
            ...
            if _fail_mins < 60:
                ...
                lines.append(f"Failing for: {_fail_mins // 60}h {_fail_mins % 60}m — sustained failure")
            else:
                lines.append(f"Failing for: {_fail_mins // 1440}d — CHRONIC failure, likely config issue not runtime")
        else:
...
def _section_fix_history(db: Any, app_key: str, cutoff: int, lines: list[str]) -> None:
    try:
        ...
        if fixes:
            ...
            for f in fixes:
                ts = datetime.fromtimestamp(f["created_at"], tz=timezone.utc).strftime("%m-%d")
                icon = (
                    "✓" if f["outcome"] == "success"
                    else "✗" if f["outcome"] == "failure"
                    else "👤" if f["outcome"] == "user_approved_manual"
                    else "⏳" if f["outcome"] == "pending"
                    else "?"
                )
                suffix = ""
...
def _section_pending_fixes(db: Any, app_key: str, check_name: str,
                           lines: list[str]) -> None:
    try:
        pending = db.execute(
        ...
        if pending:
            ...
            for p in pending:
                ts = datetime.fromtimestamp(p["created_at"], tz=timezone.utc).strftime("%m-%d")
                lines.append(
...
def _section_notifications(db: Any, lines: list[str]) -> None:
    try:
        ...
        if _ntfy_enabled:
            _ntfy_app = db.execute(
                "SELECT status FROM apps WHERE key='ntfy'"
            ).fetchone()
            if _ntfy_app and _ntfy_app["status"] == "running":
                lines.append(f"Notifications: ntfy running at {_ntfy_url} — users are being notified")
            elif _ntfy_app:
                lines.append(f"⚠ Notifications: ntfy is {_ntfy_app['status']} — users may NOT be receiving alerts")
            else:
                lines.append(f"Notifications: enabled (url={_ntfy_url}) but ntfy not in app registry — delivery unknown")
    except Exception:
        pass
...
def _section_suppressed_actions(db: Any, app_key: str, lines: list[str]) -> None:
    try:
        ...
        if _suppressed:
            ...
            lines.append(
                f"SUPPRESSED ACTIONS (rejected ≥3× by user — do NOT suggest these): "
                f"{', '.join(_sup_list)}"
            )
    except Exception:
        pass
...
def _section_prev_llm_diagnoses(db: Any, app_key: str, cutoff: int,
                                lines: list[str]) -> None:
    try:
        ...
        if prev_diags:
            ...
            for d in prev_diags:
                ts = datetime.fromtimestamp(d["ts"], tz=timezone.utc).strftime("%m-%d")
...
def _section_infra_overview(db: Any, lines: list[str]) -> None:
    try:
        slots = db.execute(
            "SELECT slot, provider, status FROM infra_slots"
        ).fetchall()
        active = [f"{s['slot']}={s['provider']}" for s in slots
...
def _section_maintenance(db: Any, app_key: str, check_name: str,
                         lines: list[str]) -> None:
    try:
        mw = db.execute(
...
def _profile_cpu(cpu: dict[str, Any], cpu_model: str, cpu_cores: int,
                 lines: list[str]) -> None:
...
def _profile_docker(docker_info: dict[str, Any], total_gb: float,
                    lines: list[str]) -> None:
...
def _profile_user(user_info: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.append(
        f"Server file owner: PUID={puid} PGID={pgid}"
    ...
    try:
        ...
        if _pct > 80:
            lines.append(
                ...
                + ("CRITICAL — SQLite DB and compose fragments at risk" if _pct > 95
...
def _section_source_availability(db: Any, app_key: str, lines: list[str]) -> None:
    try:
        ...
        if src:
            lines.append(
                f"Docker image: {src['status']} — {src['error'] or 'unavailable'}"
            )
    except Exception:
        pass
...
_ARR_APPS = frozenset({"sonarr", "radarr", "lidarr", "readarr", "whisparr",
                       "prowlarr", "bazarr", "mylar3"})
...
def _section_ms_test(app_key: str, lines: list[str]) -> None:
    try:
        import json as _jt, pathlib as _pt
        from backend.core.config import config as _cfg
        ...
        if len(_runs) >= 3:
            ...
            _prev_names = {f["name"] for run in _runs[-3:-1]
                           for f in run.get("failures", [])}
            _repeated = _fail_names & _prev_names
            if _repeated:
                lines.append(f"  Repeated test failures (last 3 runs): "
                             f"{'; '.join(list(_repeated)[:3])}")
        _app_failures = [f["name"] for f in _last.get("failures", [])
                         if app_key and app_key in f.get("detail", "")]
        if _app_failures:
            lines.append(f"  ms-test failures related to {app_key}: "
                         f"{', '.join(_app_failures[:3])}")
        _orphan_fails = [f for f in _last.get("failures", [])
                         if "orphan" in f["name"].lower()]
        if _orphan_fails:
            lines.append("  ⚠ ms-test detected orphaned DB records last run "
...
def _section_infra_app_health(db: Any, lines: list[str]) -> None:
    try:
        _infra_apps = db.execute(
            "SELECT key, display_name, status, last_healthy_at "
            "FROM apps WHERE tier=0 ORDER BY key"
        ).fetchall()
        if not _infra_apps:
            return
        _infra_statuses = []
        for _ia in _infra_apps:
            ...
            _infra_statuses.append(
                f"{_ia['key']}={_ia['status']}{_age}"
            )
        lines.append("Infra app health: " + ", ".join(_infra_statuses))

        _infra_down = [i for i in _infra_apps
                       if i["status"] not in ("running", "installed", "active")]
        if _infra_down:
            lines.append(
...
def _section_ms_update_context(lines: list[str]) -> None:
    try:
        import json as _ju, pathlib as _pu
        from backend.core.config import config as _cfg
        ...
        if _changed and _ran_mins < 120:
            ...
            if _api_changes:
                lines.append(
                    ...
                    f"{', '.join(_api_changes[:5])}"
                    + ("…" if len(_api_changes) > 5 else "")
                )
    except Exception:
        pass
...
def _section_ms_update_overdue(lines: list[str]) -> None:
    try:
        ...
        if _pending:
            lines.append(
                "Overdue manual commands: " + "; ".join(_pending)
            )
    except Exception:
        pass
