...
Implements the spine's ``remediate(findings) -> list[Decision]`` seam.  This
gate is **advisory-only by CONSTRUCTION** (review R6/R7): for each ``DRIFT``
finding it consults a deterministic mapping to compute *what action it WOULD
...
def remediate(findings: list[Finding]) -> list[Decision]:
    """Advisory-only remediate seam: one Decision per DRIFT finding.
    ...
    for f in findings:
        ...
        decisions.append(
            Decision(
                finding_id=f.id,
                would_propose=_propose_action(f),
