...
Filters for events on mediastack-managed containers (those whose name
matches the Compose pattern `mediastack-<app_key>-<replica>`).
...
def _extract_app_key(container_name: str) -> Optional[str]:
    """Extract app_key from a mediastack Compose container name.

    Expected format: ``mediastack-<app_key>-<replica>`` (the default
    Compose naming: project=mediastack, service=<app_key>, replica=1).
    Leading ``/`` is stripped (Docker API sometimes includes it).

    Returns None if the name does not match — the event is ignored.
    """
    m = re.match(r"^/?mediastack-(.+?)-\d+$", container_name)
    if m:
        return m.group(1)
    return None
...
async def _handle_event(event: dict[str, Any]) -> None:
    ...
    name (must match mediastack Compose pattern).  On match, builds a
    ...
    if app_key is None:
        return  # not a mediastack container
...
async def start_docker_event_watcher() -> None:
    ...
    _watcher_task = asyncio.create_task(_watch_loop(), name="docker-event-watcher")
