    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        fragment = {
            ...
            "labels": [
                ...
                f"traefik.http.routers.homepage.entrypoints=websecure",
                f"traefik.http.routers.homepage.tls=true",
        ...
        try:
            ...
            if rc != 0:
                return ProviderResult.failure(
                    "Homepage failed to start.", detail=_out[:400]
                )
        except Exception as e:
            return ProviderResult.failure(f"Could not deploy Homepage: {e}")

        with StateDB() as db:
            db.update_slot("dashboard", status="active", provider="homepage",
