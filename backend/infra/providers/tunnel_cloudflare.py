...
class CloudflareTunnelProvider(InfraProvider):
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        with StateDB() as db:
            db.upsert_tunnel_provider("cloudflared",
                           status="active",
                           config={
                               "domain": domain,
            ...
            db.set_setting("cf_auto_register_hostnames",
                           "true" if auto_register else "false")
    ...
    def register_hostname(self, hostname: str, target: str) -> ProviderResult:
        ...
        try:
            ...
            if get_r.is_success:
                ...
                rules = [r for r in existing
                         if r.get("hostname") and r["hostname"] != hostname]
            else:
                log.warning("Could not fetch tunnel config (%d) — starting fresh.",
                            get_r.status_code)
                rules = []
        except Exception as e:
        ...
        try:
            put_r = httpx.put(
                f"{CF_API_BASE}/accounts/{account_id}/cfd_tunnel/{tunnel_id}/configurations",
                headers=headers, json={"config": {"ingress": ingress}}, timeout=15,
            )
        ...
        dns_id = None
        try:
            existing_dns = httpx.get(
                f"{CF_API_BASE}/zones/{zone_id}/dns_records",
                headers=headers, params={"name": hostname, "type": "CNAME"}, timeout=10,
            ).json().get("result", [])
            cname = {"type": "CNAME", "name": hostname,
                     "content": f"{tunnel_id}.cfargotunnel.com", "proxied": True}
            if existing_dns:
                dns_r = httpx.put(
                    f"{CF_API_BASE}/zones/{zone_id}/dns_records/{existing_dns[0]['id']}",
                    headers=headers, json=cname, timeout=10)
            else:
                dns_r = httpx.post(
                    f"{CF_API_BASE}/zones/{zone_id}/dns_records",
                    headers=headers, json=cname, timeout=10)
        ...
        with StateDB() as db:
            db.record_external_resource(
                "cf_tunnel_hostname", hostname=hostname, target=target,
                resource_id=dns_id, config={"tunnel_id": tunnel_id, "zone_id": zone_id},
            )
    ...
    def unregister_hostname(self, hostname: str) -> ProviderResult:
        ...
        matching = [r for r in resources
                    if r["hostname"] == hostname and r["resource_id"]]
    ...
    def list_hostnames(self) -> ProviderResult:
        ...
        hostnames = [r["hostname"] for r in resources
                     if r["resource_type"] == "cf_tunnel_hostname"]
        return ProviderResult.success(
    ...
    def pre_migration_snapshot(self) -> ProviderResult:
        ...
        return ProviderResult.success(
            ...
            data={"slot": provider_rec,
                  "hostnames": hostnames.data or {}},
        )
