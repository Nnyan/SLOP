...
class PortainerProvider(InfraProvider):
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        with StateDB() as db:
            db.update_slot("management", status="active", provider="portainer",
                           config={"port": host_port, "domain": domain})

        url = f"https://portainer.{domain}" if domain else f"http://localhost:{host_port}"
        return ProviderResult.success(
            f"Portainer CE deployed at {url}. "
            f"Create your admin account on first login."
        )
    ...
    def list_hostnames(self) -> ProviderResult:
        try:
            ...
            with _SDB_p() as _db_p:
                _db_p.upsert_app(
                    "portainer", display_name="Portainer", tier=0,
                    category="management", status="running",
                    image=IMAGE, image_tag="latest",
                    container_name=CONTAINER_NAME,
                    host_port=9000, last_healthy_at=int(_t_p.time()),
                )
        except Exception:
            pass
