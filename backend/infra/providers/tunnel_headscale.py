...
class HeadscaleProvider(InfraProvider):
    ...
    description = "Self-hosted Tailscale control server. Full WireGuard mesh — no Tailscale cloud dependency."
...
""",
        ...
        fragment = {
            ...
            "ports": [
                "8085:8080",  # Headscale API (different from Mediastack port 8080)
                "9090:9090",  # Metrics
            ],
            "labels": {
                "traefik.enable": "true",
                f"traefik.http.routers.headscale.rule": f"Host(`{domain}`)",
        ...
        with StateDB() as db:
            db.upsert_tunnel_provider(self.key, status="active",
                           display_name=self.display_name, container_name=CONTAINER_NAME)
    ...
    def verify(self) -> ProviderResult:
        ...
        try:
            ...
            return (ProviderResult.success("Headscale is running.")
                    if status == "running"
