...
class TailscaleProvider(InfraProvider):
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        Optional:
          hostname — advertised hostname on the tailnet (default: mediastack)
        ...
        hostname = cfg.get("hostname", "mediastack")
        ...
        with StateDB() as db:
            db.upsert_tunnel_provider("tailscale", status="active",
    ...
    def remove(self) -> ProviderResult:
        try:
            ...
            subprocess.run(
                ["docker", "compose", "-f", str(frag_path), "--env-file", str(config.env_file), "down"],
