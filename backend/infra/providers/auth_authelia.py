...
class AutheliaProvider(InfraProvider):
    ...
    description = "Open-source SSO + 2FA. More feature-rich than TinyAuth; supports LDAP, email OTP, TOTP."
...
""",
        ...
        fragment = {
            ...
            "labels": {
                "traefik.enable": "true",
                f"traefik.http.routers.authelia.rule": f"Host(`auth.{domain}`)",
        ...
        with StateDB() as db:
            db.update_slot("auth", provider=self.key, status="active",
    ...
    def remove(self) -> ProviderResult:
        ...
        with StateDB() as db:
            db.update_slot("auth", provider=None, status="empty",
                           display_name=None, container_name=None)
