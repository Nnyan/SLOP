...
from backend.core.compose import compose_up, compose_down,\
     write_fragment
...
class DockhandProvider(InfraProvider):
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        network = platform.network_name or "mediastack"
        ...
        if use_postgres:
            env["DATABASE_URL"] = "postgresql://dockhand:${DOCKHAND_DB_PASSWORD}@postgres:5432/dockhand"
        ...
        with StateDB() as db:
            db.update_slot("management", status="active", provider="dockhand",
    ...
    def remove(self) -> ProviderResult:
        try:
            ...
            subprocess.run(["docker", "compose", "-f", str(frag_path), "--env-file", str(config.env_file), "down"],
        ...
        try:
            ...
            with _SDB_dockhand() as _db_dockhand:
                _db_dockhand.upsert_app(
                    "dockhand", display_name="Dockhand", tier=0,
                    category="management", status="running",
                    image="fnsys/dockhand:latest", image_tag="latest",
                    container_name="dockhand",
                    host_port=8080,
...
class DockgeProvider(InfraProvider):
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        with StateDB() as db:
            db.update_slot("management", status="active", provider="dockge",
        ...
        try:
            ...
            with _SDB_dockge() as _db_dockge:
                _db_dockge.upsert_app(
                    "dockge", display_name="Dockge", tier=0,
                    category="management", status="running",
                    image="louislam/dockge:1", image_tag="latest",
                    container_name="dockge",
                    host_port=5001,
...
class KomodoProvider(InfraProvider):
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        # FerretDB needs Postgres to exist (shared managed service)
        ...
        ferretdb_frag = {
            ...
            "networks": [network],
            "environment": {
                "FERRETDB_POSTGRESQL_URL": "postgres://mediastack:${POSTGRES_PASSWORD}@postgres:5432/komodo",
            },
        }
        write_fragment("komodo-ferretdb", ferretdb_frag)

        # Periphery agent companion
        periphery_frag = {
        ...
        write_fragment("komodo-periphery", periphery_frag)
        ...
        core_fragment = {
            ...
            "environment": {
                "TZ": platform.timezone or "UTC",
                "KOMODO_HOST": f"https://komodo.{domain}" if domain else f"http://localhost:{host_port}",
                "KOMODO_TITLE": "Komodo",
        ...
        try:
            ...
            for companion_key in ("komodo-ferretdb", "komodo-periphery"):
                companion_path = config.compose_dir / f"{companion_key}.yaml"
                subprocess.run(
                    ["docker", "compose", "-f", str(companion_path), "--env-file", str(config.env_file), "up", "-d", "--pull", "missing"],
                    capture_output=True, timeout=60,
                )

            frag_path = write_fragment("komodo", core_fragment)
            rc, _out = compose_up(frag_path, timeout=90)
            if rc != 0:
                return ProviderResult.failure("Komodo Core failed to start.", detail=_out[:400])
        except Exception as e:
            return ProviderResult.failure(f"Could not deploy Komodo: {e}")

        with StateDB() as db:
            db.update_slot("management", status="active", provider="komodo",
                           config={"port": host_port, "domain": domain})
    ...
    def remove(self) -> ProviderResult:
        for key in ("komodo", "komodo-periphery", "komodo-ferretdb"):
            try:
        ...
        try:
            ...
            with _SDB_komodo() as _db_komodo:
                _db_komodo.upsert_app(
                    "komodo", display_name="Komodo", tier=0,
                    category="management", status="running",
                    image="ghcr.io/moghtech/komodo-core:latest", image_tag="latest",
                    container_name="komodo",
                    host_port=9120,
    ...
    def deploy(self, cfg: dict[str, Any]) -> ProviderResult:
        ...
        fragment = {
            ...
            "labels": [
                ...
                f"traefik.http.services.portainer.loadbalancer.server.port=9000",
            ] if domain else [],
        }
        ...
        with StateDB() as db:
            db.update_slot("management", status="active", provider="portainer_be",
