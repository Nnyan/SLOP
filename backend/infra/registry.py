...
def get_provider(slot: str, key: str) -> InfraProvider:
    ...
    if cls is None:
        ...
        raise KeyError(
            f"No provider '{key}' for slot '{slot}'. "
...
def swap_slot(
    ...
    try:
        ...
        if not deploy.ok:
            result.add("deploy_new", "error",
                       f"Could not deploy {to_provider_key}.", deploy.detail)
            result.ok = False
        ...
        result.add("migrate_config", migrate["status"], migrate["message"],
                   migrate.get("detail", ""))
        ...
        if not verify.ok:
            result.add("verify_new", "error",
                       f"{to_provider_key} is not working correctly.", verify.detail)
            result.ok = False
        ...
        result.add("complete", "ok",
                   f"Slot '{slot}' now uses {new_provider.display_name}.")
...
def _migrate_config(slot: str, old: InfraProvider,
                    new: InfraProvider) -> dict[str, str]:
    ...
    if slot == "auth":
        ...
        return {"status": "skipped",
                "message": f"No config migration defined for slot '{slot}'."}


def _rollback(result: SwapResult, old_provider: InfraProvider,
              snapshot: dict[str, Any], migration_id: int | None) -> None:
    ...
    try:
        ...
        if restore.ok:
            result.add("rollback", "ok",
                       f"Rolled back to {old_provider.display_name} successfully.")
        else:
            result.add("rollback", "error",
                       f"Rollback failed — manual intervention required.",
                       restore.detail)
    except Exception as e:
        result.add("rollback", "error",
                   "Rollback encountered an unexpected error.", str(e))
    result.rolled_back = True
    if migration_id:
        _complete_migration(migration_id, "rolled_back")


def _update_migration(migration_id: int | None,
                      steps_completed: int, current_step: str) -> None:
...
def _complete_migration(migration_id: int | None,
...
def _ensure_providers_registered() -> None:
    ...
    for cls in [
        ...
        AutheliaProvider, HeadscaleProvider,
    ]:
