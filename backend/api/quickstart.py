...
PHASES: list[dict[str, Any]] = [
    {"id": "platform",  "label": "Platform check",     "route": None,          "optional": False,
     "description": "Verify dependencies and configuration are healthy."},
    {"id": "traefik",   "label": "Deploy Traefik",      "route": "/infra",      "optional": False,
     "description": "Reverse proxy that routes traffic to all your apps."},
    {"id": "auth",      "label": "Set up auth",         "route": "/infra",      "optional": True,
     "description": "Protect your apps with TinyAuth or Authelia."},
    {"id": "tunnel",    "label": "Configure tunnel",    "route": "/infra",      "optional": True,
     "description": "Expose apps externally via Cloudflare or Tailscale."},
    {"id": "storage",   "label": "Configure storage",   "route": "/storage",    "optional": False,
     "description": "Set local media paths and optional cloud sources."},
    {"id": "routing",   "label": "Configure routing",   "route": "/routing",    "optional": False,
     "description": "Choose which apps handle movies, TV, music, etc."},
    {"id": "apps",      "label": "Install apps",        "route": "/catalog",    "optional": False,
     "description": "Install your first media stack apps via Quick Stacks."},
    {"id": "llm",       "label": "Set up AI agent",     "route": "/models",     "optional": True,
...
def _get_phases(db: Any) -> list[dict[str, Any]]:
    rows = {r["phase"]: r for r in db.execute(
        "SELECT phase, status, completed_at FROM quickstart_phases"
    ).fetchall()}
    result = []
    for p in PHASES:
        row = rows.get(p["id"], {})
        result.append({**p, "status": row.get("status", "pending"),
...
def get_quickstart() -> dict[str, Any]:
    ...
    with StateDB() as db:
        ...
        _auto_phase_checks: dict[str, Callable[[], bool]] = {
            "platform": lambda: (
                db.execute("SELECT COUNT(*) FROM infra_slots WHERE status='active'").fetchone()[0] > 0
            ...
            "tunnel": lambda: db.execute(
                "SELECT COUNT(*) FROM infra_tunnel_providers WHERE status='active'"
            ).fetchone()[0] > 0,
            "storage": lambda: db.execute(
                "SELECT COUNT(*) FROM storage_sources WHERE status='active'"
            ).fetchone()[0] > 0,
            "apps": lambda: db.execute(
                "SELECT COUNT(*) FROM apps WHERE status NOT IN ('removing','disabled')"
            ).fetchone()[0] > 0,
            "llm": lambda: db.execute(
                "SELECT COUNT(*) FROM llm_model_registry WHERE enabled=1"
            ).fetchone()[0] > 0,
        }
        _changed = False
        ...
        required_done = sum(1 for p in phases if not p["optional"] and p["status"] in ("complete", "skipped"))
