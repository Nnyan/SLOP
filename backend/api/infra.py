...
def get_slot(slot: str) -> SlotOut:
    ...
    return SlotOut(slot=s.slot, provider=s.provider, providers=[],
                   status=s.status, config=s.config, deployed_at=s.deployed_at)
...
def deploy_provider(slot: str, req: DeployRequest) -> VerifyOut:
    ...
    if slot == "tunnel":
        ...
        from backend.core.state import StateError
...
def verify_provider(slot: str, provider_key: str | None = None) -> VerifyOut:
    ...
    if slot not in _valid_slots:
        raise HTTPException(status_code=404, detail=f"Unknown slot '{slot}'. Valid: {sorted(_valid_slots)}")
    if slot == "tunnel":
        with StateDB() as db:
        ...
        target = active[0] if not provider_key else next(
            (p for p in active if p["provider"] == provider_key), None
        )
        if not target:
            raise HTTPException(status_code=404, detail=f"Tunnel provider '{provider_key}' not active.")
        try:
...
# ---------------------------------------------------------------------------
from backend.api.infra_schemas import PROVIDER_CONFIG_SCHEMAS  # noqa: E402
