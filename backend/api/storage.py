...
def get_storage_source(source_id: int) -> SourceOut:
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Storage source {source_id} not found.")


@router.delete("/{source_id}")
def delete_storage_source(source_id: int) -> dict[str, Any]:
    try:
...
def get_mount_config(source_id: int) -> MountConfigOut:
    ...
    the install_steps to activate. Mediastack does not modify the host OS
    ...
    return MountConfigOut(
        ...
        note=(
            "Mediastack generates configuration files but does not modify your host OS. "
...
def verify_storage_source(source_id: int) -> VerifyResult:
    ...
    if accessible:
        try:
            files_preview = [
                p.name for p in sorted(Path(source.mount_point).iterdir())[:10]
            ]
        except Exception:
            pass
    ...
    try:
        ...
        for item in sorted(target.iterdir())[:50]:
            try:
                ...
                entries.append({"name": item.name, "type": "unknown", "size": None, "modified": None})
        return {
            "mount_point": source.mount_point,
        ...
        raise HTTPException(status_code=403, detail=f"Permission denied reading {target}")
    except OSError as e:
