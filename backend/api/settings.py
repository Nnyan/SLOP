...
def get_settings() -> SettingsOut:
    ...
    last_at = (
        _dt.datetime.fromtimestamp(int(last_at_raw)).isoformat()
        if last_at_raw else None
    )
...
def update_settings(payload: SettingsPayload) -> SettingsOut:
    ...
    with StateDB() as db:
        ...
        if payload.health_check_interval_secs is not None:
            db.set_setting("health_check_interval_secs",
                           str(max(30, payload.health_check_interval_secs)))  # 30s minimum
        ...
        if payload.cf_auto_register_hostnames is not None:
            db.set_setting("cf_auto_register_hostnames",
...
def get_system_profile() -> dict[str, Any]:
    ...
    return {
        ...
        "os": {"distro": profile.os_distro, "version": profile.os_version,
               "arch": profile.os_arch, "kernel": profile.kernel_version},
        "cpu": {"model": profile.cpu_model, "cores": profile.cpu_cores,
                "avx2": profile.avx2},
        "ram": {"total_gb": round(profile.total_ram_mb/1024,1),
                "available_gb": round(profile.free_ram_mb/1024,1)},
        "gpu": ({"vendor": profile.gpu_vendor, "model": profile.gpu_name,
                 "vram_gb": round(profile.gpu_vram_mb/1024,1),
                 "inference_capable": profile.gpu_inference_capable}
                if profile.gpu_name else {}),
        "docker": {"engine": profile.docker_version,
                   "compose": profile.compose_version,
                   "containers_running": profile.containers_running},
        "timezone": profile.timezone,
        "server_ip": profile.server_ip,
        "user": {"puid": profile.puid, "pgid": profile.pgid,
...
def get_secrets() -> dict[str, Any]:
    ...
    sensitive = {"CF_DNS_API_TOKEN", "POSTGRES_PASSWORD", "KOMODO_JWT_SECRET",
                 "KOMODO_PASSKEY", "DOCKHAND_DB_PASSWORD", "PLEX_CLAIM"}
...
def update_ai_safety(payload: SafetyUpdate) -> dict[str, Any]:
    ...
    """
    from backend.core.ai_safety import set_safety_level, ACTABLE_TYPES
    from fastapi import HTTPException
    try:
...
def get_cloud_llm_settings() -> dict[str, Any]:
    ...
    from backend.core.cloud_llm import PROVIDERS, DEFAULT_CASCADE, _get_monthly_spend
    from backend.core.state import StateDB
    import datetime
    ...
    # Monthly spend data
    first_of_month = int(datetime.datetime(
    ...
    try:
        ...
        spend_by_provider = {r["provider"]: {"spend": r["spend"], "tokens": r["tokens"],
                                              "calls": r["calls"]} for r in spend_rows}
...
def update_cloud_llm_settings(payload: CloudLLMUpdate) -> dict[str, Any]:
    ...
    with StateDB() as db:
        ...
        if payload.monthly_limit_usd is not None:
            ...
            db.set_setting("cloud_llm_monthly_limit_usd",
                           str(round(payload.monthly_limit_usd, 2)))
    return {"ok": True}
