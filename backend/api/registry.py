...
from backend.manifests.loader import clear_cache, load_all_manifests
...
def _load_db_registry() -> list[dict[str, Any]]:
    ...
    cols = ["id", "key", "display_name", "description", "category", "icon",
            "source_url", "author", "verified", "installed", "registry_url",
...
async def sync_registry() -> SyncResult:
    ...
    try:
        ...
        return SyncResult(ok=False, added=0, updated=0, total=0,
            ...
            if existing:
                db._c.execute(
                    """UPDATE manifest_registry SET
                       ...
                       WHERE key=?""",
                    (entry.get("display_name", key), entry.get("description", ""),
                     entry.get("category", "tools"), entry.get("icon", "📦"),
                     entry.get("source_url", ""), entry.get("author", "community"),
                     1 if entry.get("verified") else 0, registry_url, key),
                )
                updated += 1
            else:
                db._c.execute(
                    """INSERT INTO manifest_registry
                       ...
                       VALUES (?,?,?,?,?,?,?,?,?,unixepoch())""",
                    (key, entry.get("display_name", key), entry.get("description", ""),
                     entry.get("category", "tools"), entry.get("icon", "📦"),
                     entry.get("source_url", ""), entry.get("author", "community"),
                     1 if entry.get("verified") else 0, registry_url),
                )
                added += 1
...
def get_registry_entry(key: str) -> RegistryEntry:
    ...
    with StateDB() as db:
        row = db._c.execute(
            "SELECT * FROM manifest_registry WHERE key=?", (key,)
        ).fetchone()
    ...
    if not row:
        ...
        try:
            ...
            pass
        raise HTTPException(status_code=404, detail=f"Registry entry '{key}' not found. Run /sync first.")
    ...
    return RegistryEntry(
        ...
        icon=r.get("icon", "📦"), tier=2, service_type="management",
        tags=[], source_url=r["source_url"],
        author=r.get("author", "community"),
        verified=bool(r.get("verified", 0)),
        installed=r["key"] in installed,
    )
...
async def pull_manifest(key: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Could not download manifest: {e}")

    # Mark as installed in registry DB
    with StateDB() as db:
        db._c.execute(
            "UPDATE manifest_registry SET installed=1 WHERE key=?", (key,)
        )
        # NOTE: StateDB auto-commits on __exit__ — db._c.commit() removed (Core Rule 4.4)
