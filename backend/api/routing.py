...
class UpdateRoutingRequest(BaseModel):
    ...
    seerr_download_id: int | None = Field(None, description="Seerr's internal ID for download instance")
...
def _load_routing(media_type: str) -> dict[str, Any]:
    ...
    cols = ["id", "media_type", "debrid_instance", "download_instance",
            "seerr_debrid_id", "seerr_download_id", "default_path", "notes", "updated_at"]
...
def _all_routing() -> list[dict[str, Any]]:
    with StateDB() as db:
        rows = db._c.execute(
            "SELECT * FROM request_routing ORDER BY media_type"
        ).fetchall()
    cols = ["id", "media_type", "debrid_instance", "download_instance",
            "seerr_debrid_id", "seerr_download_id", "default_path", "notes", "updated_at"]
...
def remove_instance(instance_key: str, delete_config: bool = False) -> dict[str, Any]:
    ...
    with StateDB() as db:
        db._c.execute(
            "DELETE FROM app_instances WHERE instance_key=?", (instance_key,)
        )
...
def get_media_routing(media_type: str) -> RoutingConfig:
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Routing not found for '{media_type}'")
...
def update_media_routing(media_type: str, req: UpdateRoutingRequest) -> RoutingConfig:
    ...
    with StateDB() as db:
        ...
        db._c.execute(
            ...
            WHERE media_type=?
            """,
            (req.debrid_instance, req.download_instance,
             req.seerr_debrid_id, req.seerr_download_id,
             req.default_path, req.notes, media_type),
        )
        # NOTE: StateDB auto-commits on __exit__ — db._c.commit() removed (Core Rule 4.4)
...
def seerr_setup_help(media_type: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"No routing configured for {media_type}")
    ...
    if media_type in SEERR_SUPPORTED:
        ...
        steps = [
            ...
            "Record these IDs and enter them in Mediastack → Routing → Media Types.",
            "",
        ...
        steps = [
            ...
            f"  Install a second instance for the other path:",
