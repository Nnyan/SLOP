...
def list_models() -> list[GGUFFileInfo]:
    ...
    1. Mediastack data/models/*.gguf  — downloaded through the UI
    ...
    try:
        ...
        req = _req.Request(f"{ollama_url}/api/tags",
        ...
        pass  # Ollama unreachable — only show local files
...
def download_model_sse(
    ...
    def _download_thread() -> None:
        try:
            ...
            for progress in gen:
                event_queue.put(("progress" if not progress.done else "done_progress",
                                 progress))
    ...
    async def _stream() -> Any:
        ...
        while True:
            ...
            if progress.done:
                ...
                # Real-time progress
                total_mb = round(progress.total_bytes / 1024 / 1024, 1) \
                    if progress.total_bytes else None
                payload = {
...
def evaluate_hardware_for_model(model_size_gb: float = 0.0,
                                quantization: str = "Q4_K_M") -> HardwareEvalResult:
    ...
    # CPU + speed
    avx_flags = " · ".join(f for f in ["AVX", "AVX2", "AVX512"] if avx.get(f.lower().replace("-",""), avx.get(f.lower().replace("-","").replace("512","512"))))
    ...
    if gpu.get("name"):
        ...
        steps.append(HardwareEvalStep(
            label="GPU", status="info",
            detail="No GPU detected — CPU-only inference"
        ))
    ...
    try:
        ...
        steps.append(HardwareEvalStep(
            label="Storage", status=st_status,
            detail=f"{free_gb:.1f} GB free{spd_label}"
        ))
        storage_free_gb = free_gb
    except Exception:
        storage_free_gb = 999.0
        steps.append(HardwareEvalStep(label="Storage", status="info", detail="Could not check disk"))
    ...
    for m in RECOMMENDED_MODELS:
        ...
        if gpu_ok:
            ...
            row_status = "error"
        model_rows.append({
            "name": m["name"], "size_gb": sz,
            "recommended_for": m["recommended_for"],
            "mode": mode, "status": row_status,
        })
    steps.append(HardwareEvalStep(
    ...
    steps.append(HardwareEvalStep(
        label="Verdict", status=verdict_status,
...
def preflight_download(url: str) -> PreflightResult:
    ...
    try:
        request = _req.Request(url, method="HEAD", headers=headers)
        with _req.urlopen(request, timeout=15) as resp:
            ...
            return PreflightResult(
                ...
                size_mb=size_mb, content_type=content_type,
                requires_auth=False, error=None,
            )
    except _uerr.HTTPError as e:
        if e.code == 401:
            return PreflightResult(
                ok=False, filename=fname, size_mb=None, content_type=None,
                requires_auth=True,
                error=(
        ...
        if e.code == 404:
            return PreflightResult(
                ok=False, filename=fname, size_mb=None, content_type=None,
                requires_auth=False,
                error=f"URL not found (404). Check the download link is correct.",
            )
        return PreflightResult(
            ok=False, filename=fname, size_mb=None, content_type=None,
            requires_auth=False,
        ...
        return PreflightResult(
            ok=False, filename=fname, size_mb=None, content_type=None,
            requires_auth=False, error=str(e),
        )
...
def search_huggingface_models(q: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"HuggingFace search failed: {e}")
    ...
    with StateDB() as db:
        db.execute(
            """INSERT OR REPLACE INTO fix_history
               ...
               VALUES (?, ?, ?, ?, ?, ?)""",
            (rec.app_key, rec.error_type, rec.context[:500],
             rec.suggested_fix[:500], rec.outcome, int(time.time())),
        )
    return {"ok": True}
...
def get_registry() -> dict[str, Any]:
    ...
    from backend.core.llm_router import (
        get_all_models, routing_table, sync_registry_with_files,
        detect_capabilities, default_task_scores,
    )
    ...
    try:
        ...
        _req_obj = _req2.Request(f"{_url}/api/tags",
                                  headers={"User-Agent": "Mediastack/3.0"})
        with _req2.urlopen(_req_obj, timeout=3) as _resp:
...
def get_routing_log(limit: int = 50) -> list[dict[str, Any]]:
    ...
    import time as _t
    with StateDB() as db:
        # Auto-create table if missing (migration safety)
        db.execute("""
