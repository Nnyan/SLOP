...
from backend.core.logging import (
    get_logger, set_correlation_id, reset_correlation_id,
)
...
class CorrelationIdMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        cid = request.headers.get(_HEADER) or request.headers.get(
            _HEADER.lower()
        ) or str(uuid.uuid4())
...
_API_NONVERSIONED_PATHS: frozenset[str] = frozenset({
    "/api/ping", "/api/health", "/api/coverage",
})
...
_AUDIT_PATH_BLOCKLIST: frozenset[str] = frozenset({
    "/metrics", "/healthz", "/readyz", "/startupz",
})
...
class AuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if (
            ...
            or request.url.path in _AUDIT_PATH_BLOCKLIST
        ):
            return response
