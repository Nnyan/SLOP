...
This is the heart of Mediastack's automation. It handles the full lifecycle:
...
def _install_clear_stale_locks() -> None:
    ...
    _now = time.time()
    stale = [k for k, t in list(_installing_started.items())
             if _now - t > MAX_INSTALL_SECONDS]
    for _sk in stale:
        _installing.discard(_sk)
        _installing_started.pop(_sk, None)
        log.warning("Cleared stale install lock for '%s' (> %ds)",
                    _sk, MAX_INSTALL_SECONDS)


def _install_load_manifest(key: str,
                           result: ExecutionResult) -> AppManifest | None:
    ...
    try:
        ...
        result.fail("load_manifest",
                    f"Could not load manifest for '{key}'.", str(e))
        return None
...
def _install_register_cf_hostname(key: str, manifest: AppManifest,
    ...
    try:
        ...
        result.steps.append(StepLog(
            name="hostname_register", status="warning",
...
def _install_autoconfig_ollama() -> None:
    ...
    Ollama runs as a container on the mediastack network, so the correct
    URL from inside the mediastack container is http://ollama:11434.
...
def _install_cleanup_failed_record(key: str, db: StateDB) -> None:
    ...
    try:
        ...
        if _frag.exists():
            ...
            log.info("Install of '%s' failed before deploy — DB record removed",
                     key)
    except Exception:
        pass


def _install_finalize(op_id: int | None, key: str,
                      result: ExecutionResult) -> None:
...
def install_app(
    ...
    try:
        _install_inner(manifest, result, extra_env, host_port_override)
    except Exception as e:
        result.fail("unexpected",
                    "An unexpected error occurred during install.", str(e))
        log.exception("Unexpected error installing %s", key)

    if result.ok:
        _install_register_cf_hostname(key, manifest, result)
        result.steps.append(_run_smoke_test(key, manifest))
...
# Apps with internal ports that conflict with Traefik (80/443) or Mediastack
...
def _validate_install(platform: Any, manifest: AppManifest, key: str,
                      existing: Any, result: ExecutionResult) -> bool:
    ...
    if existing and existing.status == "running":
        ...
        try:
            ...
            _container = None
        if _container and _container.health in ("healthy", "none") and _container.status == "running":
            result.add("validate", "ok",
                       f"{manifest.display_name} is already running and healthy.")
            result.ok = True
            return False  # success early-return
        if _container:
            log.info("Allowing retry of '%s' — container unhealthy: %s/%s",
                     key, _container.status, _container.health)

    result.add("validate", "ok", "Validation passed.")
    return True


def _install_dependencies(manifest: AppManifest, platform: Any,
                          result: ExecutionResult) -> bool:
    """Resolve postgres / redis / app / companion deps. Returns True to continue."""
    deps = manifest.dependencies

    if deps.postgres:
        pr = _ensure_managed_service("postgres")
        # "warning" (not "error") so the UI surfaces this as a wait, not failure
        step_status = pr["status"] if pr["status"] != "error" else "warning"
        result.add("deps_postgres", step_status,
    ...
    if deps.redis:
        rr = _ensure_managed_service("redis")
        step_status = rr["status"] if rr["status"] != "error" else "warning"
        result.add("deps_redis", step_status,
    ...
    for dep_key in deps.apps:
        ...
        if dep is None or dep.status != "running":
            result.fail(
                "deps_app",
                f"{manifest.display_name} requires '{dep_key}' to be running first.",
                f"Install {dep_key} before installing {manifest.display_name}.",
            )
            return False
...
def _ensure_config_dir(platform: Any, key: str,
    ...
    try:
        ...
        result.fail(
            ...
            f"Check that the user running Mediastack can write to {platform.config_root}. Error: {e}",
        )
        return None
    return config_path, created_now


def _compute_host_port(manifest: AppManifest,
                       host_port_override: int | None) -> int | None:
    """Resolve the externally-bindable host port. None when port is reserved."""
    raw_port = host_port_override or manifest.web_port
    return None if (raw_port in _SYSTEM_PORTS) else raw_port


def _check_port_conflict(key: str, host_port: int | None,
                         result: ExecutionResult) -> bool:
    """Return False (and result.fail) when host_port is held by another app.

    Checks both running containers AND DB records — a stopped app
    (status='installed') still owns its port until removed.
    """
    ...
    if _db_owner:
        result.fail(
            ...
            f"Start '{_db_owner[0]}' to confirm it still needs that port, "
            f"or remove it and retry.",
        )
        return False
    return True


def _build_compose_service(manifest: AppManifest, key: str, platform: Any,
                           host_port: int | None, config_path: Path,
    ...
    tinyauth_enabled = (
        slot_auth.provider == "tinyauth" and slot_auth.status == "active"
    )
    lan_subnet = (
        platform.config.get("lan_subnet")
        if hasattr(platform, "config") else None
    )
    extra_vols = [
    ...
    if manifest.extra_config:
        service_fragment.update(manifest.extra_config)
    return service_fragment


def _write_compose_files(key: str, service_fragment: dict[str, Any],
                         result: ExecutionResult) -> Path | None:
...
def _run_deploy(key: str, frag_path: Path, config_path: Path,
                dir_created_now: bool, result: ExecutionResult) -> bool:
    ...
    """
    deploy_result = _docker_compose_up(key, frag_path)
...
def _run_post_deploy_steps(manifest: AppManifest, platform: Any,
                           result: ExecutionResult) -> None:
    ...
    for step in manifest.post_deploy:
        ...
        result.add(f"post_{step.step_type}", sr["status"], sr["message"],
                   sr.get("detail", ""))
        if sr["status"] == "error":
            result.ok = False
...
def _register_install(manifest: AppManifest, key: str, host_port: int | None,
                      config_path: Path, extra_env: dict[str, str] | None,
                      result: ExecutionResult) -> None:
...
def _install_inner(
    ...
    service_fragment = _build_compose_service(
        manifest, key, platform, host_port, config_path, extra_env,
    )
    frag_path = _write_compose_files(key, service_fragment, result)
    if frag_path is None:
        return

    if not _run_deploy(key, frag_path, config_path, dir_created_now, result):
        return
...
def _remove_unregister_hostname(key: str, result: ExecutionResult) -> Any:
    ...
    try:
        ...
        result.add("hostname_unregister", "ok",
                   f"External hostname for '{key}' removed from tunnel.")
        return manifest
    except Exception as e:
        result.add("hostname_unregister", "warning",
...
def _remove_companions(manifest: Any, result: ExecutionResult) -> None:
    ...
    try:
        ...
        result.add("companions_remove", "warning",
...
def _remove_stop_container(key: str, result: ExecutionResult) -> None:
    ...
    if not frag_path.exists():
        result.add("stop", "skipped",
                   "No compose fragment found — container may already be stopped.")
        return
    try:
...
def _remove_unwire(app: Any, result: ExecutionResult) -> None:
    ...
    with StateDB() as db:
        if app.id:
            db.execute(
                "UPDATE wiring SET status='stale' "
                "WHERE source_app_id=? OR target_app_id=?",
                (app.id, app.id),
            )
    result.add("unwire", "ok", "Wiring entries marked stale in DB.")


def _remove_config_folder(app: Any, delete_config: bool | None,
                          result: ExecutionResult) -> None:
    ...
    if delete_config:
        try:
            ...
            result.add("config", "warning",
                       f"Could not delete config folder: {e}",
                       "Delete manually: rm -rf " + str(config_dir))
        return
    result.add("config", "skipped",
               f"Config folder retained at {config_dir}.")


def _remove_inner(key: str, app: Any, result: ExecutionResult,
    ...
    with StateDB() as db:
        db.remove_app(key)
    result.add("state", "ok", f"{app.display_name} removed from Mediastack.")
...
def replace_app(old_key: str, new_key: str,
                extra_env: dict[str, str] | None = None) -> ExecutionResult:
    ...
    # Pre-validate: check that new app's port doesn't conflict with old app's port
    # (old app is still running at this point)
    try:
        ...
        if (old_port and new_port and old_port == new_port
                and new_port not in _SYSTEM_PORTS):
            # Port will conflict — temporarily stop old app to free the port
            _old_frag = config.compose_dir / f"{old_key}.yaml"
            if _old_frag.exists():
                compose_down(_old_frag, timeout=30)
                result.add("stop_old", "ok",
                           f"Stopped {old_key} to free port {old_port} for {new_key}.")
    except Exception as _pe:
        result.add("port_precheck", "warning",
                   f"Could not pre-validate port compatibility: {_pe}")
    ...
    # Rewire: update wiring rows so dependents point to new app
    try:
        with StateDB() as db:
        ...
        result.add("rewire", "ok",
                   f"Wiring connections updated from {old_key} → {new_key}.")
    except Exception as _we:
        result.add("rewire", "warning",
                   f"Could not rewire connections: {_we}",
                   detail=str(_we))

    # Remove old app (keep config folder)
    remove_result = remove_app(old_key, delete_config=False)
    for step in remove_result.steps:
        step.name = f"remove_old_{step.name}"
        result.steps.append(step)
    if not remove_result.ok:
        result.add("remove_old", "warning",
                   f"New app is running but old app ({old_key}) removal had issues.",
                   remove_result.error)

    result.add("complete", "ok",
...
def _ensure_managed_service(service_type: str) -> dict[str, str]:
    ...
    image, tag, port = images[service_type]
    ...
    if service_type == "postgres":
        fragment["environment"] = {
            "POSTGRES_USER": "${POSTGRES_USER:-mediastack}",
    ...
    try:
        ...
        if r.returncode != 0:
            # Retry once — first pull can be slow on cold start
            r2 = subprocess.run(
...
def _deploy_companions(manifest: AppManifest, platform: Any) -> dict[str, str]:
    ...
    return {"status": "ok",
            "message": f"Deployed {len(manifest.companions)} companion service(s)."}


def _docker_compose_up(key: str, frag_path: Path) -> dict[str, str]:
    """Run docker compose up for a single service fragment."""
    try:
        rc, output = compose_up(frag_path, pull=True, timeout=120)
        if rc != 0:
            return {
                "status": "error",
...
def _run_post_deploy_step(step: Any, manifest: AppManifest,
                           platform: Any) -> dict[str, str]:
    ...
    if step.step_type == "wait_healthy":
        ...
        return {"status": "skipped", "message": "Custom steps not yet implemented."}
...
def _expand_path(path: str, platform: Any) -> str:
    ...
    return (path
            .replace("{config_root}", platform.config_root or "")
            .replace("{media_root}", platform.media_root or ""))
...
def disable_app(key: str, reason: str = "user_request") -> DisableResult:
    ...
    if criticality == Criticality.INVIOLABLE:
        return DisableResult(
            key=key, ok=False, criticality=criticality,
            error=(
    ...
    if not app:
        return DisableResult(key=key, ok=False, criticality=criticality,
                             error=f"App '{key}' is not installed.")
    ...
    import subprocess
    try:
        subprocess.run(["docker", "stop", "-t", "10", key],
    ...
    with StateDB() as db:
        ...
        db.upsert_health_check(
            "app", key, "availability",
            status="warning",
            summary=f"Disabled ({reason}). Re-enable in Mediastack UI.",
            auto_fix="enable_app",
        )
        db.log_operation(
            "disable", "app", key,
...
def enable_app(key: str) -> DisableResult:
    ...
    if not app:
        return DisableResult(key=key, ok=False, criticality=str(criticality),
                             error=f"App '{key}' is not installed.")
    ...
    if disabled_path.exists():
        ...
        return DisableResult(
            key=key, ok=False, criticality=str(criticality),
            error=(
                f"Compose fragment not found for '{key}'. "
                f"The app may need to be reinstalled."
            ),
        )
    ...
    with StateDB() as db:
        if app.id:
            db.execute(
                "UPDATE wiring SET status='pending' "
                "WHERE source_app_id=? OR target_app_id=?",
                (app.id, app.id),
            )
        ...
        db.upsert_health_check(
            "app", key, "availability",
            status="ok", summary="Re-enabled successfully.",
        )
...
def install_instance(
    ...
    if role not in ("default", "debrid", "download", "secondary"):
        return InstanceResult(
            instance_key=instance_key, manifest_key=manifest_key, ok=False, role=role,
    ...
    import copy, dataclasses as _dc2
    ...
    if host_port_override:
        ...
        if host_port_override in in_use:
            return InstanceResult(
                instance_key=instance_key, manifest_key=manifest_key, ok=False, role=role,
                error=(f"Port {host_port_override} is already in use by "
        ...
        if _db_owner:
            return InstanceResult(
                instance_key=instance_key, manifest_key=manifest_key, ok=False, role=role,
                error=(f"Port {host_port_override} is reserved by app '{_db_owner[0]}'. "
...
def _register_app_hostname(key: str, manifest: Any, platform: Any) -> StepLog:
    ...
    """
    step = StepLog(name="hostname_register", status="skipped",
...
def _run_smoke_test(key: str, manifest: Any) -> StepLog:
    ...
    step = StepLog(name="smoke_test", status="ok",
    ...
    if web_port in _SYSTEM_PORTS:
        step.status = "skipped"
        step.message = (f"System-port app (port {web_port}) — "
                        f"smoke test via Traefik is not reliable from host. "
                        f"Check: docker logs {key}")
        return step

    # Use the host-mapped port — Mediastack runs on the host, not inside Docker.
    ...
    # Use the manifest's defined health check path instead of always "/"
    health_path = http_check.path if (http_check and http_check.path) else "/"
    ...
    if http_check:
        ...
        step.message = (
            f"{manifest.display_name} port {web_port} is accepting connections."
        )
