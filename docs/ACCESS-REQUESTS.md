...
- `[ ]` **[allow] `Write(.claude/settings.local.json)` + `Edit(.claude/settings.local.json)` — settings-write permission for `tools/sanctioned/robot_settings.py`** — The sanctioned operator-handoff tool (`tools/sanctioned/robot_settings.py`, S-68 Stream C) lifts and restores deny rules in `.claude/settings.local.json` via `tools/sanctioned/_lift_restore.py`. This requires Write/Edit access to that file. Currently blocked by the existing deny entries `Edit(/home/stack/code/slop/.claude/settings.local.json)` and `Write(/home/stack/code/slop/.claude/settings.local.json)`. The sanctioned tool enforces `try/finally` lift-restore and writes an audit entry for every operation — it is the ONLY permitted mechanism for lifting denies. Requested by: S-68 Stream C (2026-05-29). Status: pending. Note: originally planned for S-67-C; descoped to S-68-C (batch-6, 2026-05-29).
- `[—]` **[allow] Silencing the SENSITIVE-FILE safety prompt for `.claude/waves/` and `.claude/run/` in acceptEdits mode — NO MECHANISM EXISTS.** Confirmed via `claude-code-guide` agent research (2026-05-29). The `.claude/` protected-path exemption list (`commands`, `agents`, `skills`, `worktrees`) is **hardcoded**. No `permissions.allowSensitivePathAccess` key, no per-project `sensitive-path-overrides.json`, no extension mechanism. The 6 allow patterns added above silence the BASH and Write/Edit PERMISSION layers but NOT the sensitive-file SAFETY layer (separate check, runs even when allow patterns match). **Workarounds (all known):** (a) `bypassPermissions` mode — fresh sessions only, doesn't reload mid-session; (b) click "Yes, and always allow access to waves/ from this project" on the prompt once per path per session (UI-driven, can't be set via settings); (c) avoid writing to these paths from acceptEdits sessions (operator-assist sessions should defer wave-file writes to fresh-session orchestrators or sanctioned tools). Source: this session 2026-05-29, after 3+ prompts on `.claude/waves/` wave-file drafts. **No further action; documents the dead-end so future sessions don't re-research.** If a Claude Code feature request is filed at `/feedback`, link it here.
