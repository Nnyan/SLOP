# Mediastack Observability

This document covers the operator-side observability surface of Mediastack v4. It complements the in-code documentation in `backend/core/logging.py` (structured logs) and `backend/core/metrics.py` (Prometheus metrics).
...
The four observability paths sit OUTSIDE the `/api/v1/` versioning umbrella because they're operational infrastructure, not the application API. They will not change shape between major Mediastack versions without an explicit deprecation cycle.
...
### Custom (Mediastack-specific)

Defined in `backend/core/metrics.py`:

| Metric | Type | Labels | Source |
|--------|------|--------|--------|
| `mediastack_install_duration_seconds` | histogram | `app_key`, `outcome` | `backend.manifests.executor.install_app()` end-to-end timing. `outcome` = `success` / `failed`. Buckets cover 1s–10min range. |
| `mediastack_health_check_duration_seconds` | histogram | `app_key`, `outcome` | `backend.health.checker.check_app()` per-invocation duration. |
| `mediastack_db_query_duration_seconds` | histogram | `verb` | StateDB query duration. `verb` = SQL verb (SELECT/INSERT/UPDATE/DELETE/DDL/OTHER). Bucketed for sub-millisecond to multi-second range. |
| `mediastack_errors_total` | counter | `endpoint`, `error_class` | Errors by endpoint + exception type name. Both labels are bounded; raw URLs / dynamic IDs are NOT used as label values. |
...
The endpoint is enabled by default. To disable in tests or other environments, the instrumentator does NOT currently respect an env var (the `should_respect_env_var` flag is off — Mediastack's metrics are cheap enough to always run). If a future deployment needs gating, flip that flag in `backend/api/main.py`.
...
**Retention:** unlimited by default. The table grows linearly with mutation volume; a homelab sees a few hundred entries per day, so this is fine for years. A future `mediastack_audit_log_rows_total` metric (custom, low-cardinality) would track this for capacity planning.
...
Three panel groups make sense for the Mediastack signal set:
...
2. **Mediastack-domain timing** — install duration p95, health check duration p95.
3. **Error rates** — `rate(http_requests_total{status=~"5.."}[5m])` and `rate(mediastack_errors_total[5m])`.
