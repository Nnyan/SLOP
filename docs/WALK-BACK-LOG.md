...
- **Where it lived:** Originally proposed as a `memory/feedback_fix_all_failures.md` repo-local memory entry (which didn't actually load because Claude Code auto-memory reads from `~/.claude/projects/`, not in-repo paths). Then promoted to `CLAUDE.md` (commit `37ea3d2`, 2026-05-27). Reverted (commit `d56c1d9`, 2026-05-27).
