...
**Status:** Active — terminology of record for the Mediastack project.
...
- **tier** → [tier (cleanup or hardening phase)](#tier-cleanup-or-hardening-phase), [tier (ms-enforce gate level)](#tier-ms-enforce-gate-level), [tier (rate-limit category)](#tier-rate-limit-category)
...
**Scope.** `docs/cleanup/PROJECT_CLEANUP.md` and `docs/cleanup/HARDENING_V4_2_PLAN.md` sub-task lists; equivalent v5+ plans.

**Authoritative source.** `PROJECT_CLEANUP.md` and `HARDENING_V4_2_PLAN.md` "Model assignments" sections (identical wording).
...
**Authoritative source.** `docs/RELEASE_PROCESS.md` (section 3 documents all four pre-release gates; section 4 documents the vocabulary sweep procedure); this glossary for canonical disambiguation.

**Aliases.** [release-tag gate](#release-tag-gate); tag-gate; tag-gate review.

**Do not confuse with.** [ms-enforce gate](#ms-enforce-gate) (commit-time architectural checks; a different release-process checkpoint), [pre-flight verification](#pre-flight-verification) (release-tag-time tag-prompt checklist; another distinct checkpoint).

**See also.** [ms-enforce gate](#ms-enforce-gate), [pre-flight verification](#pre-flight-verification), [drift pattern](#drift-pattern).
...
**Authoritative source.** `docs/cleanup/PROJECT_CLEANUP.md`.
...
**Definition.** A numbered architectural rule in `docs/CORE_RULES.md`, structured under Sections 1–5 (verifiable rules) or Section 6 (guidance). Each verifiable Core Rule pairs with an `ms-coverage` rule entry naming the proving test function.
...
**Authoritative source.** Core Rule 5.21; `tools/commit_msg_hook.py`; `ms-changelog` `CUTOFF_SHA` constant.
...
**Definition.** The v4.2 phase of Mediastack: extending the systematic-enforcement pattern from code-level rules (the cleanup arc) to the documentation and filesystem layer. Bookkeeping discipline plus structural enforcement, not new architectural rules.

**Scope.** `docs/cleanup/HARDENING_V4_2_PLAN.md` and its three tiers (filesystem hygiene, multi-document atomic enforcement, vocabulary discipline).
...
**Definition.** The `installer/` Python package plus the `install.sh` bootstrap wrapper and the `mediastack` CLI subcommand suite. The bootstrap (`install.sh`) downloads the repo, sets up a virtualenv, and delegates to `installer/main.py`. The subcommand suite exposes `install`, `uninstall`, `purge`, `clean`, `status`, and `smoke` operations. The package namespace is `installer.*`; external callers (audit-gate tools) import from it directly rather than spawning subprocess invocations.
...
**Scope.** `docs/cleanup/PROJECT_CLEANUP.md`, `docs/cleanup/HARDENING_V4_2_PLAN.md`, any v5+ plan adopting the same structure.

**Authoritative source.** `tools/check_cleanup_ledger.py` enforces; `HARDENING_V4_2_PLAN.md` Step 2.5 designed the linter.
...
### mediastack.managed / mediastack.app-key

**Definition.** The two Docker label keys unconditionally applied to every mediastack-managed container and volume by `backend/core/compose.py::build_service_fragment` at compose-fragment write time:
- `mediastack.managed=true` — marks the resource for `purge` (U6/U7 enumeration) and `clean` (§C.2/§C.3 enumeration).
- `mediastack.app-key=<key>` — identifies the owning catalog app; used by `clean` for per-app fidelity and by `purge` to distinguish orphan containers.

The `mediastack.*` label namespace is project-reserved. Third-party labels in this namespace risk collision with purge/clean enumeration (ADR 0017 §D.4).
...
**Do not confuse with.** [ms-enforce gate](#ms-enforce-gate), [audit gate](#audit-gate), [pre-flight verification](#pre-flight-verification) — release-process checkpoints, not model assignment. Model gate is per-sub-task; the other three are per-commit or per-release.
...
**Scope.** Toolchain (`/srv/mediastack/ms-audit`); Core Rule 5.2 (enforcement layer); Core Rule 5.13 (AI context assembly).
...
**See also.** [audit (general)](#audit-general), [audit gate](#audit-gate), [ms-enforce gate](#ms-enforce-gate).

---

### ms-enforce gate

**Definition.** Commit-time architectural enforcement via the `ms-enforce` tool, organized into Tier 0 (Semgrep rules from `.semgrep/rules/`), Tier 1 (mandatory pre-commit checks), Tier 2 (warning-only follow-ups). Run via `ms-enforce --fast` before every commit; full `ms-enforce` in CI.

**Scope.** Every commit; CI workflow `enforce.yml`; the pre-commit hook installed by `ms-setup-tools`.

**Authoritative source.** `ms-enforce` source; Core Rule 5.2; per-check Core Rules cite their `check_*` function (e.g., 4.11 → `check_snapshots`, 4.12 → `check_mock_discipline`).

**Do not confuse with.** [audit gate](#audit-gate) (release-time, not commit-time; independent review, not automated check), [pre-flight verification](#pre-flight-verification) (release-time tag-prompt checklist), [model gate](#model-gate) (per-sub-task model-tag check).

**See also.** [tier (ms-enforce gate level)](#tier-ms-enforce-gate-level), [audit gate](#audit-gate), [pre-flight verification](#pre-flight-verification).
...
**Scope.** `/srv/mediastack/ms-test.py` and `ms-test-all`; cf. ADR 0007.
...
**Definition.** The per-instance Mediastack state — the database row, configuration, and deployed-app inventory managed by `backend/platform/`. "Platform-not-configured" means the fresh-install state with no platform row; "platform reset" is the wizard-driven teardown-and-redo path.
...
**Definition.** The post-install handoff artifact written by the installer at `<install_dir>/POST_INSTALL.txt` (default: `/opt/mediastack/POST_INSTALL.txt`). Written only on a successful install with a passing smoke test. Contains service URL, default credentials, first-steps commands, and support pointers. Owner `mediastack:mediastack`, mode 0644. Its presence is a file-system signal that the install was healthy (S2a); absence alongside a state file at `phase: installed` signals smoke-test failure (S2b). Not operator-editable; the installer is the sole writer.
...
**Definition.** Release-tag-time checklist run from the tag-creation prompt template. Includes structural audit clean (`HARDENING_V4_2_PLAN.md` Step 1.4.i), vocabulary sweep (Step 3.2.b), `ms-enforce --fast` clean, pytest clean. Distinct from the audit gate (which produces a deliverable review doc) and from the ms-enforce gate (which is per-commit).

**Scope.** `docs/RELEASE_PROCESS.md` section 3.2; v4.x.x and later tag creation.

**Authoritative source.** `docs/RELEASE_PROCESS.md` section 3.2; this glossary for canonical disambiguation.

**Do not confuse with.** [audit gate](#audit-gate), [ms-enforce gate](#ms-enforce-gate), [model gate](#model-gate).

**See also.** [audit gate](#audit-gate), [ms-enforce gate](#ms-enforce-gate).
...
**Scope.** All Claude Code sessions for Mediastack v4.2+.
...
**See also.** [audit gate](#audit-gate), [ms-enforce gate](#ms-enforce-gate), [pre-flight verification](#pre-flight-verification).
...
**Scope.** `ms-coverage` source; `data/coverage_map.json`.
...
**Definition.** A pattern-based architectural rule in `.semgrep/rules/core-rules.yml`, enforcing Core Rules at the syntactic level — e.g., `bare-db-commit`, `unsanitized-user-key-as-path`, `result-add-not-fail`. Run by `ms-semgrep` and the `ms-enforce` Tier 0 gate.
...
**See also.** [Core Rule](#core-rule), [ms-enforce gate](#ms-enforce-gate).
...
**Definition.** A numbered division of a cleanup or hardening tier in `PROJECT_CLEANUP.md` or `HARDENING_V4_2_PLAN.md` (e.g., "Step 1.1," "Step 3.1"). Each step has lettered sub-tasks (1.1.a, 1.1.b, ...) and is closed by a ledger entry after `ms-enforce --fast` and pytest pass.
...
**See also.** [ms-enforce gate](#ms-enforce-gate), [Core Rule](#core-rule).
...
**Preferred form.** "cleanup Tier 2" or "v4.2 Tier 2" when context isn't obvious — distinguishes from ms-enforce tier.

**Do not confuse with.** [tier (ms-enforce gate level)](#tier-ms-enforce-gate-level) — same word, completely different mechanism. [tier (rate-limit category)](#tier-rate-limit-category) — another use.

**See also.** [step (cleanup sub-division)](#step-cleanup-sub-division), [cleanup arc](#cleanup-arc), [hardening (v4.2)](#hardening-v42).

---

### tier (ms-enforce gate level)

**Definition.** A level within the `ms-enforce` tool's check organization: Tier 0 (Semgrep architectural rules from `.semgrep/rules/`), Tier 1 (mandatory pre-commit blocking checks), Tier 2 (warning-only post-pre-commit checks). The numbering is independent of cleanup tiers and has different cardinality (three levels, including Tier 0).

**Scope.** `ms-enforce` source; Core Rules that cite "ms-enforce Tier N" (e.g., Rule 4.11 — `check_snapshots` is Tier 2).

**Authoritative source.** `ms-enforce` source.

**Preferred form.** Always qualified: "ms-enforce Tier 2."

**Do not confuse with.** [tier (cleanup or hardening phase)](#tier-cleanup-or-hardening-phase), [tier (rate-limit category)](#tier-rate-limit-category).
...
**Do not confuse with.** [tier (cleanup or hardening phase)](#tier-cleanup-or-hardening-phase), [tier (ms-enforce gate level)](#tier-ms-enforce-gate-level).
...
- **U4:** `getent passwd mediastack` returns nonzero (subject to §A.6 pre-existing-user carve-out)
- **U4b:** `getent group mediastack` returns nonzero or group has no unexpected members (subject to §A.6.5 carve-out)
- **U5a:** data dir preserved and untouched — inode and mtime unchanged (*uninstall* only)
- **U5b:** `<data_dir>` does not exist (*purge* only)
- **U6:** `docker ps -a --filter label=mediastack.managed=true` returns empty (*purge* only)
- **U7:** `docker volume ls --filter label=mediastack.managed=true` returns empty (*purge* only)

U1–U3 failures stop the pipeline; U4, U4b, U5–U7 failures are reported and make the exit nonzero but the rest of the pipeline continues. `clean` has inverted polarity: U1, U2, U3 *violated* is expected (mediastack keeps running); U6 and U7 *hold* (managed containers/volumes removed).

**Scope.** `installer/uninstall.py`; ADR 0017 §B.2; INV-12, INV-13, INV-14.

**Authoritative source.** ADR 0017 §B.2.

**See also.** [verify_removed()](#verify_removed), [mediastack.managed / mediastack.app-key](#mediastackmanaged--mediastackapp-key), [INV-N framework](#inv-n-framework).
