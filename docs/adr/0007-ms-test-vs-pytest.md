...
- **Coverage is split.** `ms-coverage` (the rule-coverage scanner) sees pytest tests for unit/integration coverage and `ms-test.py` sections for cross-layer coverage. Both feed the same coverage map (generated by `ms-enforce --list`).
